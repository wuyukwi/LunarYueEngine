...
property_layout::~property_layout() { pop_layout(); }
...
void inspector::before_inspect(const rttr::property& prop) { layout_ = std::make_unique<property_layout>(prop); }

void inspector::after_inspect(const rttr::property& prop) { layout_.reset(); }

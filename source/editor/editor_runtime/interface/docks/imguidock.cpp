...
namespace imguidock
{
    ...
    bool dockspace::dock_to(dock* ddock, slot dock_slot, float size, bool active) { return dock_with(ddock, nullptr, dock_slot, size, active); }
        ...
        if (dock_to != nullptr)
        {
            if (dock_slot == imguidock::slot::tab)
            {
                dock_to->container->active_dock = active                                    ? ddock :
                                                  current_container->splits[0]->active_dock ? current_container->splits[0]->active_dock :
                                                                                              ddock;
                ...
                else
                {
                    ...
                    if (dock->container->parent->splits[0] == dock->container)
                    {
                        ...
                        if (dock->container->parent->parent->splits[0] == dock->container->parent)
                            ...
                            working = dock->container->parent->parent->splits[1] = dock->container->parent->splits[1];
                    ...
                    else
                    {
                        ...
                        if (dock->container->parent->parent->splits[0] == dock->container->parent)
                            ...
                            working = dock->container->parent->parent->splits[1] = dock->container->parent->splits[0];
                        working->parent = parent;
                        working->size   = dock->container->parent->size;
                    }
                }
            ...
            else
            {
                if (active_dock->draging)
                {
                    if (active_dock->redock_to)
                    {
                        ...
                        active_dock->redock_from->dock_with(active_dock, active_dock->redock_to, active_dock->redock_slot, 0, true);
                        active_dock->redock_to   = nullptr;
            ...
            if (dragged_window_id != INVALID_WINDOW)
            {
                ...
                if (dockspace.root.splits[0]->active_dock != container->active_dock)
                {
                        ...
                        (mouse_pos[1] > screen_cursor_pos.y && mouse_pos[1] < (screen_cursor_pos.y + size.y)))
                    {
                        ...
                        slot dock_slot = render_dock_slot_preview(dragged_window_id, cursor_pos, screen_cursor_pos, size);
                        ImGui::PopClipRect();
                        ImGui::EndChild();
    ...
    slot dockspace::render_dock_slot_preview(std::uint32_t id, const ImVec2& mouse_pos, const ImVec2& cPos, const ImVec2& cSize)
    {
        ...
        col_preview.w      = 0.3f;
        auto check_slot    = [convex_slot_preview, &col, &mouse_pos](ImRect rect, slot slot, std::vector<ImVec2>& convex) -> bool {
            if (convex_slot_preview)
            {
                get_slot_convex(rect, slot, convex);
                if (!convex.empty())
                {
                    ImGui::GetWindowDrawList()->AddPolyline(convex.data(), 4, ImGui::ColorConvertFloat4ToU32(col), true, 2.0f);
                }
            ...
            else
            {
                ...
                ImGui::GetWindowDrawList()->AddRectFilled(slot_rect.Min, slot_rect.Max, ImGui::ColorConvertFloat4ToU32(col));
    ...
    void dockspace::render_tab_bar(node* container, const ImVec2& sz, const ImVec2& cursor_pos, float tabbar_height)
    {
        ...
        for (auto dock : container->docks)
        {
            ...
            ImVec4 button_color       = ImVec4(childBg.x - 0.04f, childBg.y - 0.04f, childBg.z - 0.04f, childBg.w * 0.6f);
            ...
            ImVec2       title_sz         = ImGui::CalcItemSize(
                ImVec2(0, tabbar_height), title_label_size.x + style.FramePadding.x * 2.0f, title_label_size.y + style.FramePadding.y * 2.0f);

            const ImVec2 close_label_size = ImGui::CalcTextSize("X", NULL, true);
            ImVec2       close_sz         = ImGui::CalcItemSize(
                ImVec2(0, tabbar_height), close_label_size.x + style.FramePadding.x * 2.0f, close_label_size.y + style.FramePadding.y * 2.0f);
    ...
    void dock::initialize(const std::string& dtitle, bool close_btn, const ImVec2& min_sz, std::function<void(const ImVec2&)> ddrawFunction)
    {
        static int i  = 0;

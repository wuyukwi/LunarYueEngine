...
void process_vertices(aiMesh* mesh, mesh::load_data& load_data)
{
    ...
    for (size_t i = 0; i < mesh->mNumVertices; ++i, current_vertex_ptr += vertex_stride)
    {
        ...
        if (mesh->mTextureCoords[0] != nullptr && has_texcoord0)
        {
            ...
            if (has_texcoord0)
            {
                gfx::vertex_pack(textureCoords, true, gfx::attribute::TexCoord0, load_data.vertex_format, current_vertex_ptr);
            }
        }
        ...
        if (mesh->mNormals != nullptr && has_normal)
        {
            ...
            if (has_normal)
            {
                gfx::vertex_pack(math::value_ptr(normal), true, gfx::attribute::Normal, load_data.vertex_format, current_vertex_ptr);
            }
        }
        ...
        if (mesh->mTangents != nullptr && has_tangent)
        {
            ...
            if (has_tangent)
            {
                gfx::vertex_pack(math::value_ptr(tangent), true, gfx::attribute::Tangent, load_data.vertex_format, current_vertex_ptr);
            }
        }
        ...
        if (mesh->mBitangents != nullptr && has_bitangent)
        {
            ...
            if (has_bitangent)
            {
                gfx::vertex_pack(math::value_ptr(bitangent), true, gfx::attribute::Bitangent, load_data.vertex_format, current_vertex_ptr);
            }
        }
    }
}
...
void process_bones(aiMesh* mesh, std::uint32_t subset_offset, mesh::load_data& load_data)
{
    if (mesh->mBones != nullptr)
    {
        ...
        for (size_t i = 0; i < mesh->mNumBones; ++i)
        {
            ...
            auto it = std::find_if(
                std::begin(bone_influences), std::end(bone_influences), [&bone_name](const auto& bone) { return bone_name == bone.bone_id; });
...
void process_imported_scene(const aiScene* scene, mesh::load_data& load_data, std::vector<runtime::animation>& animations)
{
...
bool importer::load_mesh_data_from_file(const std::string& path, mesh::load_data& load_data, std::vector<runtime::animation>& animations)
{
    ...
    const aiScene* scene = importer.ReadFile(
        path,
        aiProcess_ConvertToLeftHanded | aiProcess_CalcTangentSpace | aiProcess_GenSmoothNormals | aiProcess_JoinIdenticalVertices |
            aiProcess_ImproveCacheLocality | aiProcess_LimitBoneWeights | aiProcess_RemoveRedundantMaterials | aiProcess_SplitLargeMeshes |
            aiProcess_Triangulate | aiProcess_GenUVCoords | aiProcess_SortByPType | aiProcess_FindDegenerates | aiProcess_FindInvalidData |

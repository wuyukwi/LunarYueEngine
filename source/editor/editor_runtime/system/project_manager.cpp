...
namespace editor
{
    ...
    static std::uint64_t watch_assets(const fs::path& dir, const std::string& wildcard, bool reload_async)
    {
        ...
        return fs::watcher::watch(watch_dir, true, true, 500ms, [&am, &ts](const auto& entries, bool is_initial_list) {
            for (const auto& entry : entries)
            {
                ...
                if (entry.type == fs::file_type::regular)
                {
                    ...
                    else if (entry.status == fs::watcher::entry_status::renamed)
                    {
                        ...
                        auto task         = ts.push_on_owner_thread([old_key, key, &am]() { am.rename_asset<T>(old_key, key); });
                    }
                    else
                    {
    ...
    template<typename T>
    static void add_to_syncer(std::vector<uint64_t>&                watchers,
                              fs::syncer&                           syncer,
                              const fs::path&                       dir,
                              ...
                              const fs::syncer::on_entry_renamed_t& on_renamed)
    {
        ...
        auto  on_modified = [&ts](const auto& ref_path, const auto& synced_paths, bool is_initial_listing) {
            auto task = ts.push_on_worker_thread([ref_path, synced_paths = remove_meta_tag(synced_paths), is_initial_listing]() {
    ...
    template<>
    void add_to_syncer<gfx::shader>(std::vector<uint64_t>&                watchers,
                                    fs::syncer&                           syncer,
                                    const fs::path&                       dir,
                                    const fs::syncer::on_entry_removed_t& on_removed,
                                    const fs::syncer::on_entry_renamed_t& on_renamed)
    {
        ...
        auto on_modified = [&ts](const auto& ref_path, const auto& synced_paths, bool is_initial_listing) {
            auto task = ts.push_on_worker_thread([ref_path, synced_paths = remove_meta_tag(synced_paths), is_initial_listing]() {
                ...
                auto        it = std::find_if(std::begin(synced_paths), std::end(synced_paths), [&renderer_extension](const auto& key) {
        ...
        {
            syncer.set_mapping(
                type + ".meta", {".dx11.asset", ".dx12.asset", ".gl.asset", ".vlk.asset"}, on_modified, on_modified, on_removed, on_renamed);
    ...
    bool project_manager::open_project(const fs::path& project_path)
    {
        ...
        setup_cache_syncer(app_watchers_, app_cache_syncer_, fs::resolve_protocol("app:/meta"), fs::resolve_protocol("app:/cache"));
    ...
    void project_manager::setup_meta_syncer(fs::syncer& syncer, const fs::path& data_dir, const fs::path& meta_dir)
    {
    ...
    void project_manager::setup_cache_syncer(std::vector<uint64_t>& watchers, fs::syncer& syncer, const fs::path& meta_dir, const fs::path& cache_dir)
    {
        setup_directory(syncer);
    ...
    {
        load_config();
        setup_meta_syncer(engine_meta_syncer_, fs::resolve_protocol("engine:/data"), fs::resolve_protocol("engine:/meta"));
        setup_cache_syncer(engine_watchers_, engine_cache_syncer_, fs::resolve_protocol("engine:/meta"), fs::resolve_protocol("engine:/cache"));
        setup_meta_syncer(editor_meta_syncer_, fs::resolve_protocol("editor:/data"), fs::resolve_protocol("editor:/meta"));
        setup_cache_syncer(editor_watchers_, editor_cache_syncer_, fs::resolve_protocol("editor:/meta"), fs::resolve_protocol("editor:/cache"));
    }

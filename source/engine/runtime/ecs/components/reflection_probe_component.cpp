...
int reflection_probe_component::compute_projected_sphere_rect(irect32_t&             rect,
                                                              ...
                                                              const math::transform& proj)
{
    ...
    {
        return math::compute_projected_sphere_rect(rect.left, rect.right, rect.top, rect.bottom, position, probe_.sphere_data.range, view, proj);
    }
    else if (probe_.type == probe_type::box)
    {
        ...
        return math::compute_projected_sphere_rect(rect.left, rect.right, rect.top, rect.bottom, position, d, view, proj);
    }
    else
    {
        return 1;
...
std::shared_ptr<gfx::texture> reflection_probe_component::get_cubemap()
{
    static auto buffer_format =
        gfx::get_best_format(BGFX_CAPS_FORMAT_TEXTURE_FRAMEBUFFER | BGFX_CAPS_FORMAT_TEXTURE_CUBE | BGFX_CAPS_FORMAT_TEXTURE_MIP_AUTOGEN,
...
std::shared_ptr<gfx::frame_buffer> reflection_probe_component::get_cubemap_fbo() { return render_view_[0].get_fbo("CUBEMAP", {get_cubemap()}); }

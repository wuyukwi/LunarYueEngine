...
{
    bool all_valid = std::all_of(std::begin(shaders_), std::end(shaders_), [](auto& shader) { return shader && shader->is_valid(); });
...
void gpu_program::set_texture(uint8_t _stage, const std::string& _sampler, gfx::frame_buffer* _fbo, uint8_t _attachment, uint32_t _flags)
{
    program_->set_texture(_stage, _sampler, _fbo, _attachment, _flags);
}

void gpu_program::set_texture(uint8_t _stage, const std::string& _sampler, gfx::texture* _texture, uint32_t _flags)
{
    program_->set_texture(_stage, _sampler, _texture, _flags);
}

void gpu_program::set_uniform(const std::string& _name, const void* _value, uint16_t _num) { program_->set_uniform(_name, _value, _num); }
...
std::shared_ptr<gfx::uniform> gpu_program::get_uniform(const std::string& _name, bool texture) { return program_->get_uniform(_name, texture); }

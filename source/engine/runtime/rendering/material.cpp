...
gpu_program* material::get_program() const { return skinned ? program_skinned_.get() : program_.get(); }
...
standard_material::standard_material()
{
    ...
    auto f1 = ts.push_or_execute_on_owner_thread(
        [this](asset_handle<gfx::shader> vs, asset_handle<gfx::shader> fs) { program_skinned_ = std::make_unique<gpu_program>(vs, fs); },

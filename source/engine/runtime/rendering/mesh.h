...
public:
    ...
    get_skinning_matrices(const std::vector<math::transform>& node_transforms, const skin_bind_data& bind_data, bool compute_inverse_transpose) const;
    ...
    /// </summary>
    //-----------------------------------------------------------------------------
    void compute_palette_fit(bone_index_map_t& input, std::int32_t& current_space, std::int32_t& common_base, std::int32_t& additional_bones);
...
protected:
    ...
    struct bone_combination_key
    {
        ...
        std::uint32_t    data_group_id = 0;
        // Constructor
        bone_combination_key(face_influences* _influences, std::uint32_t group_id) : influences(_influences), data_group_id(group_id) {}
    };
    ...
    /// </summary>
    //-----------------------------------------------------------------------------
    void bind_mesh_data(std::uint32_t face_start, std::uint32_t face_count, std::uint32_t vertex_start, std::uint32_t vertex_count);

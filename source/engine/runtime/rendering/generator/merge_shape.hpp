    ...
    public:
        ...
        private:
            ...
            explicit edges_t(const merge_shape_t& shape) :
                head_ {shape.head_.edges()}, tail_(shape.tail_.edges()), head_vertex_count_ {count(shape.head_.vertices())},
                all_done_ {tail_.done() && head_.done()}
            {}
        ...
        private:
            ...
            explicit vertices_t(const merge_shape_t& shape) :
                head_ {shape.head_.vertices()}, tail_(shape.tail_.vertices()), all_done_ {tail_.done() && head_.done()}
            {}

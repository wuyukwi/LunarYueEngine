    ...
    public:
        ...
        private:
            ...
            explicit triangles_t(const extrude_mesh_t& mesh) :
                mesh_ {&mesh}, shape_edges_ {mesh.shape_.edges()}, path_edges_ {mesh.path_.edges()}, odd_ {true}
            {}
        ...
        public:
            mesh_vertex_t generate() const
            {
                ...
                vertex.position         = pathVertex.position + shapeVertex.position[0] * pathVertex.normal + shapeVertex.position[1] * pathBinormal;
        ...
        private:
            ...
            explicit vertices_t(const extrude_mesh_t& mesh) :
                mesh_ {&mesh}, shape_vertices_ {mesh.shape_.vertices()}, path_vertices_ {mesh.path_.vertices()}
            {}
        ...
        extrude_mesh_t(shape_t shape, path_t path) :
            shape_ {std::move(shape)}, path_ {std::move(path)}, shape_vertex_count_ {count(shape_.vertices())}
        {}

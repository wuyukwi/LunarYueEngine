    ...
    public:
        ...
        private:
            ...
            explicit edges_t(const repeat_path_t* repeatPath) noexcept :
                repeat_path_ {repeatPath}, edges_ {repeatPath->path_.edges()}, index_ {repeatPath->vertex_count_ > 0 ? 0 : repeatPath->instances_},
                delta_ {0}
            {}
        ...
        explicit repeat_path_t(path_t path, int instances, const gml::dvec3& delta) noexcept :
            path_ {std::move(path)}, instances_ {instances}, delta_ {delta}, vertex_count_ {count(path_.vertices())}
        {}

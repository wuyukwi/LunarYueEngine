    ...
    public:
        spherify_mesh_t(mesh_t mesh, double radius, double factor) :
            transform_mesh_ {std::move(mesh), [radius, factor](mesh_vertex_t& value) {
                                 value.position = gml::mix(value.position, radius * gml::normalize(value.position), factor);
                                 value.normal   = gml::normalize(gml::mix(value.normal, gml::normalize(value.position), factor));
                             }}
        {}

...
static const std::array<gml::ivec3, 20> triangles {{{0, 11, 5},  {0, 5, 1},  {0, 1, 7},  {0, 7, 10}, {0, 10, 11}, {1, 5, 9}, {5, 11, 4},
                                                    {11, 10, 2}, {10, 7, 6}, {7, 1, 8},  {3, 9, 4},  {3, 4, 2},   {3, 2, 6}, {3, 6, 8},
                                                    {3, 8, 9},   {4, 9, 5},  {2, 4, 11}, {6, 2, 10}, {8, 6, 7},   {9, 8, 1}}};

icosahedron_mesh_t::triangles_t::triangles_t(const icosahedron_mesh_t& mesh) :
    mesh_ {&mesh}, i_ {0}, triangle_mesh_ {std::make_shared<triangle_mesh_t>(::vertices[::triangles[0][0]],
...
bool icosahedron_mesh_t::triangles_t::done() const noexcept { return i_ == static_cast<int>(::triangles.size()); }
    ...
    if (triangles_.done())
    {
        ...
        if (!done())
        {
            triangle_mesh_ = std::make_shared<triangle_mesh_t>(
                ::vertices[::triangles[i_][0]], ::vertices[::triangles[i_][1]], ::vertices[::triangles[i_][2]], mesh_->segments_);
...
bool icosahedron_mesh_t::vertices_t::done() const noexcept { return i_ == static_cast<int>(::triangles.size()); }
...
icosahedron_mesh_t::icosahedron_mesh_t(double radius, int segments) :
    radius_ {radius}, segments_ {segments}, face_vertex_count_ {count(triangle_mesh_t {1.0, segments}.vertices())}
{}

icosahedron_mesh_t::triangles_t icosahedron_mesh_t::triangles() const noexcept { return triangles_t {*this}; }

icosahedron_mesh_t::vertices_t icosahedron_mesh_t::vertices() const noexcept { return vertices_t {*this}; }

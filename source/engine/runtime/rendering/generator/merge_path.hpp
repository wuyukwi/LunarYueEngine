    ...
    public:
        ...
        private:
            ...
            explicit edges_t(const merge_path_t& path) :
                head_ {path.head_.triangles()}, tail_(path.tail_.triangles()), head_vertex_count_ {count(path.head_.vertices())},
                all_done_ {tail_.done() && head_.done()}
            {}
        ...
        private:
            ...
            explicit vertices_t(const merge_path_t& path) :
                head_ {path.head_.vertices()}, tail_(path.tail_.vertices()), all_done_ {tail_.done() && head_.done()}
            {}

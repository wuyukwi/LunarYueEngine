...
static const gml::dvec3 teapotData[32][4][4] = {{{{1.40, 0.00, 2.40}, {1.40, -0.78, 2.40}, {0.78, -1.40, 2.40}, {0.00, -1.40, 2.40}},
...
teapot_mesh_t::triangles_t::triangles_t(const teapot_mesh_t& mesh) noexcept :
    mesh_t {&mesh}, index_ {0}, patch_mesh_ {std::make_shared<bezier_mesh_t<4, 4>>(teapotData[0], gml::ivec2 {mesh.segments_, mesh.segments_})},
...
void teapot_mesh_t::triangles_t::next()
{
    ...
    if (triangles_.done())
    {
        ...
        if (!done())
        {
            patch_mesh_ = std::make_shared<bezier_mesh_t<4, 4>>(teapotData[index_], gml::ivec2 {mesh_t->segments_, mesh_t->segments_});
...
teapot_mesh_t::vertices_t::vertices_t(const teapot_mesh_t& mesh) noexcept :
    mesh_ {&mesh}, index_ {0}, patch_mesh_ {std::make_shared<bezier_mesh_t<4, 4>>(teapotData[0], gml::ivec2 {mesh.segments_, mesh.segments_})},
...
void teapot_mesh_t::vertices_t::next()
{
    ...
    if (vertices_.done())
    {
        ...
        if (!done())
        {
            patch_mesh_ = std::make_shared<bezier_mesh_t<4, 4>>(teapotData[index_], gml::ivec2 {mesh_->segments_, mesh_->segments_});
...
teapot_mesh_t::teapot_mesh_t(int segments) noexcept :
    segments_ {segments}, patch_vertex_count_ {count(bezier_mesh_t<4, 4> {teapotData[0], {segments, segments}}.vertices())}
{}

teapot_mesh_t::triangles_t teapot_mesh_t::triangles() const noexcept { return triangles_t {*this}; }

...
math::bbox camera::get_local_bounding_box()
{
    if (projection_mode_ == projection_mode::perspective)
    {
        ...
        return math::bbox(-far_size * aspect_ratio_, -far_size, near_clip_, far_size * aspect_ratio_, far_size, far_clip_);
    }
...
bool camera::is_aspect_locked() const { return (get_projection_mode() == projection_mode::orthographic || aspect_locked_); }
...
const math::transform& camera::get_projection() const
{
    ...
    else if (get_projection_mode() == projection_mode::orthographic)
    {
        if (projection_dirty_ || aspect_dirty_)
        {
            ...
            math::mat4 projection = ortho_(rect.left * zoom, rect.right * zoom, rect.bottom * zoom, rect.top * zoom, get_near_clip(), get_far_clip());
            projection[2][0] += aa_data_.z;
            projection[2][1] += aa_data_.w;
            projection_ = projection;
            // Matrix has been updated
...
void camera::look_at(const math::vec3& vEye, const math::vec3& vAt) { look_at(vEye, vAt, math::vec3(0.0f, 1.0f, 0.0f)); }
...
const math::frustum& camera::get_frustum() const
{
    ...
    if (frustum_dirty_ && !frustum_locked_)
    {
        ...
        clipping_volume_.points[math::volume_geometry_point::left_top_far] = clipping_volume_.points[math::volume_geometry_point::left_top_near];
        ...
        clipping_volume_.points[math::volume_geometry_point::right_top_far]    = clipping_volume_.points[math::volume_geometry_point::right_top_near];
...
bool camera::viewport_to_world(const math::vec2& point, const math::plane& pl, math::vec3& world_pos, bool clip) const
{
    ...
    if (clip && ((point.x < viewport_pos_.x) || (point.x > (viewport_pos_.x + viewport_size_.width)) || (point.y < viewport_pos_.y) ||
                 (point.y > (viewport_pos_.y + viewport_size_.height))))
    {
        return false;
    }
...
bool camera::viewport_to_major_axis(const math::vec2& point, const math::vec3& Origin, math::vec3& world_pos, math::vec3& major_axis) const
{
...
float camera::estimate_zoom_factor(const math::plane& pl) const
{
    ...
    viewport_to_world(math::vec2(float(viewport_size_.width) / 2.0f, float(viewport_size_.height) / 2.0f), pl, world, false);
...
/// </summary>
//-----------------------------------------------------------------------------
float camera::estimate_zoom_factor(const math::vec3& world_pos) const { return estimate_zoom_factor(world_pos, std::numeric_limits<float>::max()); }
...
math::vec3 camera::estimate_pick_tolerance(float wire_tolerance, const math::vec3& pos, const math::transform& object_transform) const
{
...
void camera::set_aa_data(const usize32_t& viewport_size, std::uint32_t current_subpixel_index, std::uint32_t temporal_aa_samples)
{

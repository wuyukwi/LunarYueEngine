...
bool mesh::bind_skin(const skin_bind_data& bind_data)
{
    ...
    for (std::uint32_t i = 0; i < preparation_data_.triangle_count; ++i)
    {
        ...
        for (std::uint32_t j = 0; j < 3; ++j)
        {
            ...
            {
                used_bones.bones[static_cast<std::uint32_t>(influence)] = 1; // Just set to any value, we're only using a key/value dictionary
                                                                             // for acceleration
            }
        ...
        if (it_combination == bone_combinations.end())
        {
            ...
            bone_combinations.insert(bone_combination_map_t::value_type(bone_combination_key(influences_ptr, tri_data_group_id), face_list_ptr));
    ...
    if (vertex_format_.m_hash != original_format.m_hash)
    {
        ...
        gfx::vertex_convert(vertex_format_, &preparation_data_.vertex_data[0], original_format, &original_buffer[0], original_vertex_count);
    ...
    for (size_t i = 0; i < vertex_table.size(); ++i)
    {
        ...
        if (i >= original_vertex_count)
        {
            ...
            memcpy(src_vertices_ptr + (i * vertex_stride), src_vertices_ptr + (data.original_vertex * vertex_stride), vertex_stride);
        ...
        gfx::vertex_pack(math::value_ptr(blend_weights), false, gfx::attribute::Weight, vertex_format_, src_vertices_ptr, std::uint32_t(i));

        gfx::vertex_pack(math::value_ptr(blend_indices), false, gfx::attribute::Indices, vertex_format_, src_vertices_ptr, std::uint32_t(i));
    ...
    if (prepare_status_ == mesh_status::prepared)
    {
        ...
        if (skin_bind_data_.has_bones())
        {
            ...
            for (const auto& palette : bone_palettes_)
            {
                ...
                for (std::int32_t face = subset_ptr->face_start; face < (subset_ptr->face_start + static_cast<std::int32_t>(subset_ptr->face_count));
                     ++face)
                {
                    ...
                    {
                        float weights[4];
                        gfx::vertex_unpack(weights, gfx::attribute::Weight, vertex_format_, vertex_data_ptr, tri.indices[i]);
                        float indices[4];
                        gfx::vertex_unpack(indices, gfx::attribute::Indices, vertex_format_, vertex_data_ptr, tri.indices[i]);
...
bool mesh::set_vertex_source(void* source_ptr, std::uint32_t vertex_count, const gfx::vertex_layout& source_format)
{
    ...
    else
    {
        ...
        gfx::vertex_convert(
            vertex_format_, preparation_data_.vertex_source, source_format, reinterpret_cast<std::uint8_t*>(source_ptr), vertex_count);
    } // End if !matching
...
static void create_mesh(const gfx::vertex_layout& format, const generator::any_mesh& mesh, mesh::preparation_data& data, math::bbox& bbox)
{
    ...
    for (const auto& v : mesh.vertices())
    {
        ...
        if (has_position)
            gfx::vertex_pack(&position.x, false, gfx::attribute::Position, format, current_vertex_ptr, std::uint32_t(i));
        if (has_normals)
            gfx::vertex_pack(&normal.x, true, gfx::attribute::Normal, format, current_vertex_ptr, std::uint32_t(i));
        if (has_texcoord0)
            gfx::vertex_pack(&texcoords0.x, true, gfx::attribute::TexCoord0, format, current_vertex_ptr, std::uint32_t(i));
...
bool mesh::sort_mesh_data(bool optimize, bool hardware_copy, bool build_buffer)
{
    ...
    auto sort_predicate = [](const subset* lhs, const subset* rhs) { return lhs->data_group_id < rhs->data_group_id; };
...
void mesh::bind_mesh_data(std::uint32_t face_start, std::uint32_t face_count, std::uint32_t vertex_start, std::uint32_t vertex_count)
{
    (void)vertex_start;
...
bool mesh::generate_vertex_normals(std::uint32_t* adjacency_ptr, std::vector<std::uint32_t>* remap_array_ptr /* = nullptr */)
{
    ...
    for (i = 0; i < preparation_data_.triangle_count; ++i)
    {
        ...
        const auto*     v1  = reinterpret_cast<const math::vec3*>(src_vertices_ptr + (tri.indices[0] * vertex_stride) + position_offset);
        const auto*     v2  = reinterpret_cast<const math::vec3*>(src_vertices_ptr + (tri.indices[1] * vertex_stride) + position_offset);
        const auto*     v3  = reinterpret_cast<const math::vec3*>(src_vertices_ptr + (tri.indices[2] * vertex_stride) + position_offset);
    ...
    for (i = 0; i < preparation_data_.triangle_count; ++i)
    {
        ...
        for (j = 0; j < 3; ++j)
        {
            ...
            if (!force_normal_generation_ && (preparation_data_.vertex_flags[index] & preparation_data::source_contains_normal))
                continue;
            ...
            else
            {
                    ...
                    math::abs(ref_normal.z - vec_normal.z) >= 1e-3f)
                {
                    ...
                    gfx::vertex_pack(math::value_ptr(norm), true, gfx::attribute::Normal, vertex_format_, src_vertices_ptr, index);
...
bool mesh::generate_vertex_tangents()
{
    ...
    for (i = 0; i < num_verts; i++, src_vertices_ptr += vertex_stride)
    {
        ...
        if (force_tangent_generation_ || (!has_bitangent && requires_bitangents))
        {
            ...
            // Store.
            gfx::vertex_pack(&math::vec4(B, 1.0f)[0], true, gfx::attribute::Bitangent, vertex_format_, src_vertices_ptr);
...
bool mesh::generate_adjacency(std::vector<std::uint32_t>& adjacency)
{
    ...
    else
    {
        ...
        for (std::uint32_t i = 0; i < face_count_; ++i, src_indices_ptr += 3)
        {
            ...
            // Retrieve positions of each referenced vertex.
            const auto* v1 = reinterpret_cast<const math::vec3*>(src_vertices_ptr + (src_indices_ptr[0] * vertex_stride));
            const auto* v2 = reinterpret_cast<const math::vec3*>(src_vertices_ptr + (src_indices_ptr[1] * vertex_stride));
            const auto* v3 = reinterpret_cast<const math::vec3*>(src_vertices_ptr + (src_indices_ptr[2] * vertex_stride));
...
irect32_t mesh::calculate_screen_rect(const math::transform& world, const camera& cam) const
{
    ...
    return irect32_t(irect32_t::value_type(min.x), irect32_t::value_type(min.y), irect32_t::value_type(max.x), irect32_t::value_type(max.y));
}
...
const std::vector<skin_bind_data::bone_influence>& skin_bind_data::get_bones() const { return bones_; }
...
{
    auto it = std::find_if(std::begin(bones_), std::end(bones_), [name](const auto& bone) { return name == bone.bone_id; });
...
/// </summary>
//-----------------------------------------------------------------------------
bone_palette::bone_palette(std::uint32_t palette_size) : data_group_id_(0), maximum_size_(palette_size), maximum_blend_index_(-1) {}
...
bone_palette::bone_palette(const bone_palette& rhs) :
    bones_lut_(rhs.bones_lut_), bones_(rhs.bones_), faces_(rhs.faces_), data_group_id_(rhs.data_group_id_), maximum_size_(rhs.maximum_size_),
...
bool operator<(const mesh::mesh_subset_key& key1, const mesh::mesh_subset_key& key2) { return key1.data_group_id < key2.data_group_id; }

...
namespace hpp
{
    ...
    class delegate<R(A...)>
    {
        ...
        {
            simple_type_wrapper(F val) : value(val) {}
        ...
        public:
            constexpr static const bool stored_locally =
                (is_location_invariant<F>::value && sizeof(F) <= max_size && alignof(F) <= max_align && (max_align % alignof(F) == 0));
            ...
            using is_any_member_pair = std::integral_constant<bool, is_const_member_pair<F>::value || is_member_pair<F>::value>;

            using is_function_pointer =
                std::integral_constant<bool, std::is_pointer<F>::value && std::is_function<typename std::remove_pointer<F>::type>::value>;

            static void* get_pointer(const any_data& source) { return const_cast<F*>(get_pointer_typed(source)); }

            static void clone(any_data& dest, const any_data& source) { clone_impl(dest, source, is_copiable()); }

            static bool compare(const any_data& data1, const any_data& data2) { return compare_impl(data1, data2, is_any_member_pair()); }

            static void destroy(any_data& victim) { destroy_impl(victim, local_storage()); }

            static void init(any_data& data, F&& f) { init_impl(data, std::move(f), local_storage()); }
        ...
        private:
            ...
            static void clone_impl(any_data&, const any_data&, std::false_type) { assert(false && "TRYING TO COPY A NON-COPYABLE FUNCTOR/FUNCTION"); }

            static void clone_impl(any_data& dest, const any_data& source, std::true_type) { clone_impl_local(dest, source, local_storage()); }
            ...
            static bool compare_impl(const any_data& data1, const any_data& data2, std::true_type) { return compare_impl_pair(data1, data2); }
            ...
            // destruction.
            static void destroy_impl(any_data& victim, std::true_type) { victim.template access<F>().~F(); }

            // Destroying an object located on the heap.
            static void destroy_impl(any_data& victim, std::false_type) { delete victim.template access<F*>(); }

            static void init_impl(any_data& data, F&& f, std::true_type) { new (data.access()) F(std::move(f)); }

            static void init_impl(any_data& data, F&& f, std::false_type) { data.template access<F*>() = new F(std::move(f)); }
        };
        ...
        struct manager
        {
            constexpr manager(get_pointer_type getter, clone_type cloner, compare_type comparer, destroy_type destructor) :
                get_pointer_type_(getter), clone_type_(cloner), compare_type_(comparer), destroy_type_(destructor)
            {}
    ...
    public:
        ...
        delegate(delegate&& d) { d.swap(*this); }
        ...
        delegate(T&& f)
        {
            ...
            static_assert(handler::is_copiable::value || handler::is_movable::value, "Functor must be at least copiable or movable");

            if (handler::not_empty_function(f))
            {
                handler::init(functor_, std::move(f));

                constexpr static const manager man {&handler::get_pointer, &handler::clone, &handler::compare, &handler::destroy};
    ...
    private:
        ...
        template<typename T>
        static typename std::enable_if<!(is_member_pair<T>::value || is_const_member_pair<T>::value), R>::type functor_stub(void* const object_ptr,
                                                                                                                            A... args)
        {
...
struct std::hash<hpp::delegate<R(A...)>>
{

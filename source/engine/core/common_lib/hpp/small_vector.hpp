...
#define I_HPP_SMALL_VECTOR_OUT_OF_RANGE_IF(cond) \
    if (cond) \
    throw std::out_of_range("hpp::small_vector out of range")
...
#define I_HPP_SMALL_VECTOR_OUT_OF_RANGE_IF(cond) \
    ...
    { \
        if (cond) \
        { \
            assert(false && "hpp::small_vector out of range"); \
            throw std::out_of_range("hpp::small_vector out of range"); \
        } \
    } while (false)
#else
...
namespace hpp
{
    ...
    {
        static_assert(RevertToStaticBelow <= StaticCapacity + 1,
                      "hpp::small_vector: the RevertToStaticBelow shouldn't exceed the static capacity by more than one");
    ...
    public:
        ...
        small_vector(const Alloc& alloc) : Alloc(alloc), capacity_(StaticCapacity) { begin_ = end_ = static_begin_ptr(); }

        explicit small_vector(size_t count, const Alloc& alloc = Alloc()) : small_vector(alloc) { resize(count); }

        explicit small_vector(size_t count, const T& value, const Alloc& alloc = Alloc()) : small_vector(alloc) { assign_fill(count, value); }

        template<class InputIterator, typename = decltype(*std::declval<InputIterator>())>
        small_vector(InputIterator first, InputIterator last, const Alloc& alloc = Alloc()) : small_vector(alloc)
        {
            assign_copy(first, last);
        }

        small_vector(std::initializer_list<T> l, const Alloc& alloc = Alloc()) : small_vector(alloc) { assign_move(l.begin(), l.end()); }

        small_vector(const small_vector& v) : small_vector(v, atraits::select_on_container_copy_construction(v.get_allocator())) {}

        small_vector(const small_vector& v, const Alloc& alloc) : small_vector(alloc) { assign_copy(v.begin(), v.end()); }

        small_vector(small_vector&& v) noexcept : Alloc(std::move(v.get_alloc())), capacity_(v.capacity_) { take_impl(v); }
        ...
        void shrink_to_fit()
        {
            ...
            if (s == capacity_)
                return; // we're at max
            if (is_static())
                return; // can't shrink static buf
        ...
        void pop_back() { shrink_at(end_ - 1, 1); }
        ...
        bool is_static() const { return begin_ == static_begin_ptr(); }

    private:
        const T* static_begin_ptr() const { return reinterpret_cast<const_pointer>(static_data_ + 0); }

        T* static_begin_ptr() { return reinterpret_cast<pointer>(static_data_ + 0); }
    ...
    template<typename T,
             size_t StaticCapacityA,
             size_t RevertToStaticBelowA,
             class AllocA,
             size_t StaticCapacityB,
             size_t RevertToStaticBelowB,
             class AllocB>

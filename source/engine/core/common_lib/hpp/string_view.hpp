...
namespace STX_NAMESPACE_NAME
{
...
#if 0
    {
        ...
        basic_string_view(const std::basic_string<charT, traits, Allocator>& str) noexcept : ptr_(str.data()), len_(str.length())
        {}

        constexpr basic_string_view(const charT* str) : ptr_(str), len_(traits_impl::length(str)) {}

        constexpr basic_string_view(const charT* str, size_type len) : ptr_(str), len_(len) {}
        ...
        STX_CONSTEXPR14 int compare(size_type pos1, size_type n1, basic_string_view x) const { return substr(pos1, n1).compare(x); }

        STX_CONSTEXPR14 int compare(size_type pos1, size_type n1, basic_string_view x, size_type pos2, size_type n2) const
        {
            return substr(pos1, n1).compare(x.substr(pos2, n2));
        }

        STX_CONSTEXPR14 int compare(const charT* x) const { return compare(basic_string_view(x)); }

        STX_CONSTEXPR14 int compare(size_type pos1, size_type n1, const charT* x) const { return substr(pos1, n1).compare(basic_string_view(x)); }
        ...
        template<class It>
        static constexpr // required since C++17
            typename std::iterator_traits<It>::difference_type
        ...
        template<class It>
        static constexpr // since C++17
            typename std::iterator_traits<It>::difference_type
            constexpr_distance(It first, It last)
        {
            return constexpr_do_distance(first, last, typename std::iterator_traits<It>::iterator_category());
        }
        template<class ForwardIt1, class ForwardIt2, class BinaryPredicate>
        static constexpr // since C++17
            ForwardIt1
            constexpr_search(ForwardIt1 first, ForwardIt1 last, ForwardIt2 s_first, ForwardIt2 s_last, BinaryPredicate p)
        {
            for (;; ++first)
            {
                ForwardIt1 it = first;
        ...
        STX_CONSTEXPR14 size_type find(basic_string_view s, size_type pos = 0) const noexcept
        {
            ...
            if (s.empty())
                return pos;
            const_iterator iter = constexpr_search(this->cbegin() + pos, this->cend(), s.cbegin(), s.cend(), traits_impl::eq);
            return iter == this->cend() ? npos : constexpr_distance(this->cbegin(), iter);
        }
        STX_CONSTEXPR14 size_type find(charT c, size_type pos = 0) const noexcept { return find(basic_string_view(&c, 1), pos); }
        STX_CONSTEXPR14 size_type find(const charT* s, size_type pos, size_type n) const noexcept { return find(basic_string_view(s, n), pos); }
        STX_CONSTEXPR14 size_type find(const charT* s, size_type pos = 0) const noexcept { return find(basic_string_view(s), pos); }
        ...
        STX_CONSTEXPR14 size_type rfind(charT c, size_type pos = npos) const noexcept { return rfind(basic_string_view(&c, 1), pos); }
        STX_CONSTEXPR14 size_type rfind(const charT* s, size_type pos, size_type n) const noexcept { return rfind(basic_string_view(s, n), pos); }
        STX_CONSTEXPR14 size_type rfind(const charT* s, size_type pos = npos) const noexcept { return rfind(basic_string_view(s), pos); }
        ...
        STX_CONSTEXPR14 size_type find_first_of(basic_string_view s, size_type pos = 0) const noexcept
        {
            if (pos >= len_ || s.len_ == 0)
                return npos;
            const_iterator iter = std::find_first_of(this->cbegin() + pos, this->cend(), s.cbegin(), s.cend(), traits_impl::eq);
            return iter == this->cend() ? npos : std::distance(this->cbegin(), iter);
        }
        STX_CONSTEXPR14 size_type find_first_of(charT c, size_type pos = 0) const noexcept { return find_first_of(basic_string_view(&c, 1), pos); }
        STX_CONSTEXPR14 size_type find_first_of(const charT* s, size_type pos, size_type n) const noexcept
        {
            return find_first_of(basic_string_view(s, n), pos);
        }
        STX_CONSTEXPR14 size_type find_first_of(const charT* s, size_type pos = 0) const noexcept { return find_first_of(basic_string_view(s), pos); }
        ...
        STX_CONSTEXPR14 size_type find_last_of(basic_string_view s, size_type pos = npos) const noexcept
        {
            ...
            if (pos >= len_)
                ...
                pos = len_ - (pos + 1);
            const_reverse_iterator iter = std::find_first_of(this->crbegin() + pos, this->crend(), s.cbegin(), s.cend(), traits_impl::eq);
            return iter == this->crend() ? npos : reverse_distance(this->crbegin(), iter);
        }
        STX_CONSTEXPR14 size_type find_last_of(charT c, size_type pos = npos) const noexcept { return find_last_of(basic_string_view(&c, 1), pos); }
    ...
    inline bool operator==(basic_string_view<charT, traits> x, const std::basic_string<charT, traits, Allocator>& y) noexcept
    {
        return x == basic_string_view<charT, traits>(y);
    }

    template<typename charT, typename traits, typename Allocator>
    inline bool operator==(const std::basic_string<charT, traits, Allocator>& x, basic_string_view<charT, traits> y) noexcept
    {
    ...
    // Inserter
    template<class charT, class traits>
    inline std::basic_ostream<charT, traits>& operator<<(std::basic_ostream<charT, traits>& os, const basic_string_view<charT, traits>& str)
    {
        if (os.good())
        {
            const std::size_t size = str.size();
...
// numeric conversions
    ...
    inline namespace literals
    {
        inline namespace string_view_literals
        {
            ...
            inline constexpr basic_string_view<char> operator""_sv(const char* __str, size_t __len) { return basic_string_view<char> {__str, __len}; }

            inline constexpr basic_string_view<wchar_t> operator""_sv(const wchar_t* __str, size_t __len)
            {
                return basic_string_view<wchar_t> {__str, __len};
            }

            inline constexpr basic_string_view<char16_t> operator""_sv(const char16_t* __str, size_t __len)
            {
                return basic_string_view<char16_t> {__str, __len};
            }

            inline constexpr basic_string_view<char32_t> operator""_sv(const char32_t* __str, size_t __len)
            {

...
namespace hpp
{
    ...
    struct sentinel
    {
        ...
        template<typename T>
        sentinel(const std::shared_ptr<T>& ptr) : sentinel(std::weak_ptr<T>(ptr))
        {}

        template<typename T>
        sentinel(const std::weak_ptr<T>& wptr)
        {
            predicate_ = [wptr]() { return wptr.expired(); };
        }

        template<typename Predicate>
        sentinel(const Predicate& predicate) : predicate_(predicate)
        {}
    ...
    namespace internal
    {
        ...
        template<typename T>
        typename std::enable_if<is_shared_ptr<T>::value, void>::type inline add_sentinel(predicate_list& out, const T& sent)
        {
            using PtrInternalType = typename std::decay_t<decltype((*std::declval<T>().get()))>;
            out.emplace_back([wsent = std::weak_ptr<PtrInternalType>(sent)]() { return wsent.expired(); });
        }

        template<typename T>
        typename std::enable_if<!is_shared_ptr<T>::value, void>::type inline add_sentinel(predicate_list& out, const T& sent)
        {
            out.emplace_back([sent]() { return sent.expired(); });
        }
    } // namespace internal
    ...
    inline sentinel make_sentinel(const Args&... args)
    {
        ...
        hpp::for_each(tuple, [&expiration_check_functions](const auto& el) { internal::add_sentinel(expiration_check_functions, el); });

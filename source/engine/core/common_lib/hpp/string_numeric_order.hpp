...
namespace hpp
{
    ...
    inline int numcmp(const std::string& a, const std::string& b) { return numcmp(a.data(), a.data() + a.size(), b.data(), b.data() + b.size()); }

    inline int numcmp(const char* a, const char* b) { return numcmp(a, a + strlen(a), b, b + strlen(b)); }

    struct string_numeric_order
    {
        bool operator()(const std::string& x, const std::string& y) const { return numcmp(x, y) < 0; }

        bool operator()(const char* x, const char* y) const { return numcmp(x, y) < 0; }
    };
} // namespace hpp

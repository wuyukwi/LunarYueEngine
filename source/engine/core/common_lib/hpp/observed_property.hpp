...
namespace hpp
{
    ...
    struct observed_property
    {
        ...
        observed_property(const observed_property& rhs) : value_(rhs.value_) {}

        observed_property(const T& rhs) : value_(rhs) {}
        ...
        on_change_event& on_change() { return on_change_; }
    ...
    private:
        ...
        static_assert((std::is_trivially_copyable<T>::value || std::is_assignable<T, T>::value) && !std::is_reference<T>::value &&
    ...
    template<typename T,
             typename = std::enable_if_t<(std::is_arithmetic<T>::value && !std::is_floating_point<T>::value) || operator_existence::equal<T>::value>>
    ...
    template<
        typename T,
        typename = std::enable_if_t<(std::is_arithmetic<T>::value && !std::is_floating_point<T>::value) || operator_existence::not_equal<T>::value>>
    bool operator!=(const observed_property<T>& lhs, const T& rhs)
    {
        return lhs.get() != rhs;
    }
    ...
    template<typename T,
             typename = std::enable_if_t<(std::is_arithmetic<T>::value && !std::is_floating_point<T>::value) || operator_existence::less<T>::value>>
    bool operator<(const observed_property<T>& lhs, const T& rhs)
    {
        return lhs.get() < rhs;
    }
    ...
    template<
        typename T,
        typename = std::enable_if_t<(std::is_arithmetic<T>::value && !std::is_floating_point<T>::value) || operator_existence::greater<T>::value>>
    bool operator>(const observed_property<T>& lhs, const T& rhs)
    {
        return lhs.get() > rhs;
    }
    ...
    template<typename T,
             typename =
                 std::enable_if_t<(std::is_arithmetic<T>::value && !std::is_floating_point<T>::value) || operator_existence::less_or_equal<T>::value>>
    bool operator<=(const observed_property<T>& lhs, const T& rhs)
    {
        return lhs.get() <= rhs;
    }
    ...
    template<typename T,
             typename = std::enable_if_t<(std::is_arithmetic<T>::value && !std::is_floating_point<T>::value) ||
                                         operator_existence::greater_or_equal<T>::value>>
    bool operator>=(const observed_property<T>& lhs, const T& rhs)
    {
        return lhs.get() >= rhs;
    }

...
struct char8_t_string_literal
{
    ...
    constexpr char8_t_string_literal(const char8_t (&r)[N]) : char8_t_string_literal(r, std::make_index_sequence<N>()) {}
...
constexpr char operator""_as_char(char8_t c) { return char(c); }
...
constexpr char operator""_as_char(char ch) { return ch; }

constexpr const char* operator""_as_char(const char* arr, std::size_t /*size*/) { return arr; }

...
namespace hpp
{
    ...
    public:
        ...
        explicit flat_map(const key_compare& comp, const allocator_type& alloc = allocator_type()) : cmp_(comp), container_(alloc) {}
        ...
        flat_map(std::initializer_list<value_type> init, const key_compare& comp = key_compare(), const allocator_type& alloc = allocator_type()) :
            cmp_(comp), container_(std::move(init), alloc)
        {
            std::sort(container_.begin(), container_.end(), cmp_);
            auto new_end = std::unique(
                container_.begin(), container_.end(), [this](const value_type& a, const value_type& b) { return !cmp_(a, b) && !cmp_(b, a); });
            container_.erase(new_end, container_.end());
        }

        flat_map(std::initializer_list<value_type> init, const allocator_type& alloc) : flat_map(std::move(init), key_compare(), alloc) {}
        ...
        iterator erase(iterator pos) { return container_.erase(const_iterator(pos)); }
        ...
        template<typename K>
        typename std::enable_if<std::is_convertible<K, key_type>::value, mapped_type&>::type operator[](K&& k)
        {
            auto i = lower_bound(k);
            if (i != end() && !cmp_(k, *i))
            {
        ...
        container_type& modify_container() noexcept { return container_; }

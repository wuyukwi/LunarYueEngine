...
namespace hpp
{
    ...
    public:
        ...
        template<class C>
        slot_key
        connect(C* const object_ptr, void (C::*const method_ptr)(Args...), const hpp::source_location& location = hpp::source_location::current())
        {
            return get_impl().connect_impl(hpp::nullopt, static_cast<slot_priority>(0), location, object_ptr, method_ptr);
        }
        ...
        {
            return get_impl().connect_impl(hpp::nullopt, static_cast<slot_priority>(0), location, std::forward<T>(f));
        }
        ...
        template<typename T, typename = typename std::enable_if<!std::is_same<event, typename std::decay<T>::type>::value>::type>
        slot_key connect(slot_priority priority, T&& f, const hpp::source_location& location = hpp::source_location::current())
        {
        ...
        slot_key connect(const slot_sentinel& sentinel,
                         ...
                         const hpp::source_location& location = hpp::source_location::current())
        {
            return get_impl().connect_impl(sentinel, static_cast<slot_priority>(0), location, object_ptr, method_ptr);
        }
        ...
        template<typename T, typename = typename std::enable_if<!std::is_same<event, typename std::decay<T>::type>::value>::type>
        slot_key connect(const slot_sentinel& sentinel, T&& f, const hpp::source_location& location = hpp::source_location::current())
        {
            return get_impl().connect_impl(sentinel, static_cast<slot_priority>(0), location, std::forward<T>(f));
        }
        ...
        template<typename T, typename = typename std::enable_if<!std::is_same<event, typename std::decay<T>::type>::value>::type>
        slot_key
        connect(const slot_sentinel& sentinel, slot_priority priority, T&& f, const hpp::source_location& location = hpp::source_location::current())
        {
        ...
        event(event&& rhs) noexcept { impl_ = std::move(rhs.impl_); }
    ...
    private:
        struct impl
        {
            ...
            template<typename... A>
            slot_key connect_impl(const slot_sentinel& sentinel, slot_priority priority, const hpp::source_location& location, A&&... args)
            {
                auto id = free_id_++;
        ...
        bool has_impl() const noexcept { return !!impl_; }

        impl& get_impl() const { return *impl_; }
    ...
    public:
        ...
        auto are_counts_equal() const noexcept -> bool { return call_count_ == prev_call_count_; }

        void increment_count() noexcept { call_count_++; }

        auto get_call_count() const noexcept -> uint64_t { return call_count_; }

        void equalize_counts() { prev_call_count_ = call_count_; }

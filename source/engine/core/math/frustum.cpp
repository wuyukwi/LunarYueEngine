...
namespace math
{
    ...
    /// </summary>
    //-----------------------------------------------------------------------------
    frustum::frustum(const transform& View, const transform& Proj, bool _oglNDC) { update(View, Proj, _oglNDC); }
    ...
    void frustum::recompute_points()
    {
        ...
        {
            const plane& p0 = plane::normalize((i & 1) != 0u ? planes[volume_plane::near_plane] : planes[volume_plane::far_plane]);
    ...
    bool frustum::test_extruded_aabb(const bbox_extruded& Box) const
    {
        ...
        bIntersect1 = (Box.test_line(points[volume_geometry_point::left_bottom_far], points[volume_geometry_point::left_bottom_near])) ||
    ...
    bool frustum::test_frustum(const frustum& f) const
    {
        ...
        // A -> B
        bool bIntersect1;
        bIntersect1 = test_line(f.points[volume_geometry_point::left_bottom_far], f.points[volume_geometry_point::left_bottom_near]) ||
        ...
        // B -> A
        bool bIntersect2;
        bIntersect2 = f.test_line(points[volume_geometry_point::left_bottom_far], points[volume_geometry_point::left_bottom_near]) ||
    ...
    bool frustum::operator==(const frustum& frustum) const
    {
        ...
        for (size_t i = 0; i < planes.size(); ++i)
        {
            ...
            if (!(glm::abs<float>(p1.data.x - p2.data.x) <= glm::epsilon<float>() &&
                  ...
                  glm::abs<float>(p1.data.z - p2.data.z) <= glm::epsilon<float>() && glm::abs<float>(p1.data.w - p2.data.w) <= glm::epsilon<float>()))
            {
                return false;
            }

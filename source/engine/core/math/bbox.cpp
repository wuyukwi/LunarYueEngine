...
namespace math
{
    ...
    {
        return (glm::abs<float>(max.x - min.x) < glm::epsilon<float>() && glm::abs<float>(max.y - min.y) < glm::epsilon<float>() &&
    ...
    {
        return (min.x <= bounds.max.x) && (min.y <= bounds.max.y) && (min.z <= bounds.max.z) && (max.x >= bounds.min.x) && (max.y >= bounds.min.y) &&
               (max.z >= bounds.min.z);
    }
    ...
    {
        return ((min.x - tolerance.x) <= (bounds.max.x + tolerance.x)) && ((min.y - tolerance.y) <= (bounds.max.y + tolerance.y)) &&
               ((min.z - tolerance.z) <= (bounds.max.z + tolerance.z)) && ((max.x + tolerance.x) >= (bounds.min.x - tolerance.x)) &&
               ((max.y + tolerance.y) >= (bounds.min.y - tolerance.y)) && ((max.z + tolerance.z) >= (bounds.min.z - tolerance.z));
    }
...
#define AXISTEST2(vEdge, vP0, vP1, nComponent0, nComponent1) \
    ...
    /// </summary>
    //-----------------------------------------------------------------------------
    bool bbox::operator==(const bbox& bounds) const { return (min == bounds.min) && (max == bounds.max); }

...
namespace uuids
{
    namespace detail
    {
        ...
        constexpr inline std::size_t static_strlen(const char* str) { return (str == nullptr || *str == 0) ? 0 : 1 + static_strlen(++str); }

        constexpr inline std::size_t static_wstrlen(const wchar_t* str) { return (str == nullptr || *str == 0) ? 0 : 1 + static_wstrlen(++str); }
    ...
    struct uuid
    {
        ...
        // constexpr pl0x!
        explicit uuid(const std::initializer_list<value_type>& ilist) : uuid(ilist.begin(), ilist.end()) {}
        ...
        {
            return ((data_[8] >> 5) == 0x07 ?
                        variant_future :
                        ((data_[8] >> 5) == 0x06 ?
                             variant_microsoft :
                             ((data_[8] >> 6) == 0x02 ? variant_rfc4122 : ((data_[8] >> 7) == 0x00 ? variant_ncs : variant_unknown)))); // not sure if
            // variant_unknown
            // possible... here
            // just cuz..
        }
        ...
        constexpr const_iterator end() const noexcept { return detail::static_addressof(data_[static_size - 1]); }

        constexpr const_iterator cbegin() const noexcept { return detail::static_addressof(data_[0]); }

        constexpr const_iterator cend() const noexcept { return detail::static_addressof(data_[static_size - 1]); }
        ...
        friend bool operator==(uuid const& lhs, uuid const& rhs) { return ::std::equal(lhs.begin(), lhs.end(), rhs.begin()); }

...
namespace uuids
{
    ...
    public:
        explicit random_uuid_generator(result_type seed = engine_type::default_seed) :
            generator_(seed), distribution_(std::numeric_limits<result_type>::min(), std::numeric_limits<result_type>::max())
        {}

        template<typename SeedSequence>
        explicit random_uuid_generator(SeedSequence& q) :
            generator_(q), distribution_(std::numeric_limits<result_type>::min(), std::numeric_limits<result_type>::max())
        {}

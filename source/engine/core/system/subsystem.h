...
namespace core
{
    ...
    void subsystem_context::remove_subsystem()
    {
        ...
        _orders.erase(std::remove_if(
            std::begin(_orders), std::end(_orders), [index](const auto& el) { return index == el; }, std::end(_orders)));
    }
    ...
    bool has_subsystems()
    {
        return details::status() != details::internal_status::running ? false : details::context().has_subsystems<Args...>();
    }
} // namespace core

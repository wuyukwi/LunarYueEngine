...
namespace fs
{
    ...
    namespace
    {
        ...
        auto visit_wild_card_path(const fs::path& path, bool recursive, bool visit_empty, const std::function<bool(const fs::path&)>& visitor)
                ...
                else if (fs::exists(path_filter.first, err))
                {
                    const auto iterate = [&](auto& it) {
                        for (const auto& entry : it)
                        {
                            ...
                            if ((before_pos != std::string::npos || before.empty()) && (after_pos != std::string::npos || after.empty()))
                            {
    ...
    public:
        ...
        impl(const fs::path&    path,
            ...
            filter_(filter),
            callback_(std::move(list_callback)), poll_interval_(poll_interval), recursive_(recursive)
        {
            root_ = path;
        ...
        void watch()
        {
            ...
            if (!filter_.empty())
            {
                visit_wild_card_path(root_ / filter_, recursive_, false, [this, &entries, &created, &modified](const fs::path& p) {
                    poll_entry(p, entries, created, modified);
                    return false;
                });
            }
    ...
    auto filesystem_watcher::watch(const fs::path& path, bool recursive, bool initial_list, clock_t::duration poll_interval, notify_callback callback)
        -> std::uint64_t
    {
                                        ...
                                        notify_callback&  list_callback) -> std::uint64_t
    {
        ...
        if (list_callback)
        {
            ...
            if (path.string().find('*') != std::string::npos)
            {
                auto path_filter = visit_wild_card_path(path, recursive, true, [](const fs::path&) { return true; });
            ...
            auto key = free_id++;
            {
                ...
                auto                        imp = std::make_shared<impl>(p, filter, recursive, initial_list, poll_interval, std::move(list_callback));

...
namespace fs
{
    ...
    public:
        using iterator_t = T;
        static_assert(std::is_same<iterator_t, recursive_directory_iterator>::value || std::is_same<iterator_t, directory_iterator>::value,
        ...
        cache(const fs::path& p, clock_t::duration scan_frequency) : path_(p), scan_frequency_(scan_frequency), should_refresh_(true) { watch(); }

        cache(const cache& rhs) :
            path_(rhs.path), scan_frequency_(rhs.scan_frequency_), entries_(rhs.entries_), should_refresh_(rhs.should_refresh_.load())
        {
            watch();
        }

        cache(cache&& rhs) noexcept :
            path_(std::move(rhs.path_)), scan_frequency_(std::move(rhs.scan_frequency_)), entries_(std::move(rhs.entries_)),
    ...
    private:
        ...
        void watch()
        {
            ...
            watch_id_ = watcher::watch(path_ / "*", is_recursive, false, scan_frequency_, [this](const auto&, bool) { should_refresh_ = true; });
        }

...
#include "../common_lib/hpp/type_index.hpp"
#include <functional>
#include <memory>
#include <set>
#include <sstream>
...
private:
    struct command
    {
        ...
        explicit command(const std::string&              name,
                         ...
                         const std::vector<std::string>& defaultArguments) :
            name(name),
            description(description), num_arguments(numArguments), argument_names(argumentNames), default_arguments(defaultArguments)
        {}
    ...
    bind_callback(std::function<void(T, Args...)> callback, const std::vector<std::string>& arguments, int argumentIndex);
 ...
 */
inline std::function<void()> console::bind_callback(std::function<void()> callback, const std::vector<std::string>& /*unused*/, int /*unused*/)
{
    return callback;
}

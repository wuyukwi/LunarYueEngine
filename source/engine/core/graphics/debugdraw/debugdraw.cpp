...
struct DebugShapeVertex
{
    ...
    static void init()
    {
        ms_layout.begin().add(bgfx::Attrib::Position, 3, bgfx::AttribType::Float).add(bgfx::Attrib::Indices, 4, bgfx::AttribType::Uint8).end();
    }
...
struct DebugMeshVertex
{
    ...
    static void init() { ms_layout.begin().add(bgfx::Attrib::Position, 3, bgfx::AttribType::Float).end(); }
    ...
    if (NULL != _pos0)
    {
        struct Gen
        {
            Gen(void* _pos, uint16_t _posStride, void* _normals, uint16_t _normalStride, uint8_t _subdiv) :
                m_pos((uint8_t*)_pos), m_normals((uint8_t*)_normals), m_posStride(_posStride), m_normalStride(_normalStride)
            {
...
struct GeometryT
{
    ...
    GeometryHandle create(uint32_t _numVertices, const DdVertex* _vertices, uint32_t _numIndices, const void* _indices, bool _index32)
    {
        ...
        if (isValid(handle))
        {
            ...
            geometry.m_vbh     = bgfx::createVertexBuffer(bgfx::copy(_vertices, _numVertices * sizeof(DdVertex)), DebugMeshVertex::ms_layout);

            geometry.m_topologyNumIndices[0] = _numIndices;
            geometry.m_topologyNumIndices[1] =
                bgfx::topologyConvert(bgfx::TopologyConvert::TriListToLineList, NULL, 0, _indices, _numIndices, _index32);
...
struct DebugDrawShared
{
    void init(bx::AllocatorI* _allocator)
    {
        ...
        m_program[Program::Lines] = bgfx::createProgram(bgfx::createEmbeddedShader(s_embeddedShaders, type, "vs_debugdraw_lines"),
                                                        bgfx::createEmbeddedShader(s_embeddedShaders, type, "fs_debugdraw_lines"),
                                                        true);

        m_program[Program::LinesStipple] = bgfx::createProgram(bgfx::createEmbeddedShader(s_embeddedShaders, type, "vs_debugdraw_lines_stipple"),
                                                               bgfx::createEmbeddedShader(s_embeddedShaders, type, "fs_debugdraw_lines_stipple"),
                                                               true);

        m_program[Program::Fill] = bgfx::createProgram(bgfx::createEmbeddedShader(s_embeddedShaders, type, "vs_debugdraw_fill"),
                                                       bgfx::createEmbeddedShader(s_embeddedShaders, type, "fs_debugdraw_fill"),
                                                       true);

        m_program[Program::FillMesh] = bgfx::createProgram(bgfx::createEmbeddedShader(s_embeddedShaders, type, "vs_debugdraw_fill_mesh"),
                                                           bgfx::createEmbeddedShader(s_embeddedShaders, type, "fs_debugdraw_fill"),
                                                           true);

        m_program[Program::FillLit] = bgfx::createProgram(bgfx::createEmbeddedShader(s_embeddedShaders, type, "vs_debugdraw_fill_lit"),
                                                          bgfx::createEmbeddedShader(s_embeddedShaders, type, "fs_debugdraw_fill_lit"),
                                                          true);

        m_program[Program::FillLitMesh] = bgfx::createProgram(bgfx::createEmbeddedShader(s_embeddedShaders, type, "vs_debugdraw_fill_lit_mesh"),
                                                              bgfx::createEmbeddedShader(s_embeddedShaders, type, "fs_debugdraw_fill_lit"),
                                                              true);

        m_program[Program::FillTexture] = bgfx::createProgram(bgfx::createEmbeddedShader(s_embeddedShaders, type, "vs_debugdraw_fill_texture"),
        ...
        for (uint32_t mesh = 0; mesh < 4; ++mesh)
        {
            ...
            bgfx::topologyConvert(
                bgfx::TopologyConvert::TriListToLineList, &indicesOut[numIndices], numLineListIndices * sizeof(uint16_t), trilist, numIndices, false);
    ...
    void destroy(GeometryHandle _handle) { m_geometry.destroy(_handle); }
...
struct DebugDrawEncoderImpl
{
    ...
    void begin(bgfx::ViewId _viewId, bool _depthTestLess, bgfx::Encoder* _encoder)
    {
        ...
        attrib.m_state = 0 | BGFX_STATE_WRITE_RGB | (m_depthTestLess ? BGFX_STATE_DEPTH_TEST_LESS : BGFX_STATE_DEPTH_TEST_GREATER) |
    ...
    void setTranslate(const float* _pos) { setTranslate(_pos[0], _pos[1], _pos[2]); }
    ...
    void pushTranslate(const bx::Vec3& _pos) { pushTranslate(_pos.x, _pos.y, _pos.z); }
    ...
    void setState(bool _depthTest, bool _depthWrite, bool _clockwise)
    {
        ...
        uint64_t state =
            m_attrib[m_stack].m_state & ~(0 | BGFX_STATE_DEPTH_TEST_MASK | BGFX_STATE_WRITE_Z | BGFX_STATE_CULL_CW | BGFX_STATE_CULL_CCW);
    ...
    void moveTo(Axis::Enum _axis, float _x, float _y) { moveTo(getPoint(_axis, _x, _y)); }
    ...
    void lineTo(Axis::Enum _axis, float _x, float _y) { lineTo(getPoint(_axis, _x, _y)); }
    ...
    void draw(const bx::Cylinder& _cylinder, bool _capsule) { drawCylinder(_cylinder.pos, _cylinder.end, _cylinder.radius, _capsule); }

    void draw(const bx::Disk& _disk) { drawCircle(_disk.normal, _disk.center, _disk.radius, 0.0f); }
    ...
    void draw(const bx::Sphere& _sphere)
    {
        const Attrib& attrib = m_attrib[m_stack];
        float         mtx[16];
        bx::mtxSRT(mtx, _sphere.radius, _sphere.radius, _sphere.radius, 0.0f, 0.0f, 0.0f, _sphere.center.x, _sphere.center.y, _sphere.center.z);
    ...
    void draw(bool _lineList, uint32_t _numVertices, const DdVertex* _vertices, uint32_t _numIndices, const uint16_t* _indices)
    {
        ...
        if (_numVertices == bgfx::getAvailTransientVertexBuffer(_numVertices, DebugMeshVertex::ms_layout))
        {
            ...
            if (0 < _numIndices)
            {
                ...
                {
                    numIndices = bgfx::topologyConvert(bgfx::TopologyConvert::TriListToLineList, NULL, 0, _indices, _numIndices, false);

                    bgfx::allocTransientIndexBuffer(&tib, numIndices);
                    bgfx::topologyConvert(
                        bgfx::TopologyConvert::TriListToLineList, tib.data, numIndices * sizeof(uint16_t), _indices, _numIndices, false);
                }
                else
                {
                    bgfx::allocTransientIndexBuffer(&tib, numIndices);
    ...
    void drawFrustum(const void* _viewProj) { drawFrustum((const float*)_viewProj); }
    ...
    void drawCircle(Axis::Enum _axis, float _x, float _y, float _z, float _radius, float _weight)
    {
        ...
        bx::Vec3 pos = getPoint(_axis, bx::lerp(xy0[0], xy1[0], _weight) * _radius, bx::lerp(xy0[1], xy1[1], _weight) * _radius);
        ...
        for (uint32_t ii = 1; ii < num; ++ii)
        {
            ...
            pos = getPoint(_axis, bx::lerp(xy0[0], xy1[0], _weight) * _radius, bx::lerp(xy0[1], xy1[1], _weight) * _radius);
            lineTo({pos.x + _x, pos.y + _y, pos.z + _z});
        }
        close();
    }
    ...
    void flush()
    {
        if (0 != m_pos)
        {
            if (checkAvailTransientBuffers(m_pos, DebugVertex::ms_layout, m_indexPos))
            {
                ...
                m_encoder->setVertexBuffer(0, &tvb);
                m_encoder->setIndexBuffer(&tib);
                m_encoder->setState(0 | BGFX_STATE_WRITE_RGB | BGFX_STATE_PT_LINES | attrib.m_state | BGFX_STATE_LINEAA | BGFX_STATE_BLEND_ALPHA);
                m_encoder->setTransform(m_mtxStack[m_mtxStackCurrent].mtx);
...
void ddDestroy(GeometryHandle _handle) { s_dds.destroy(_handle); }

#define DEBUG_DRAW_ENCODER(_func) reinterpret_cast<DebugDrawEncoderImpl*>(this)->_func

DebugDrawEncoder::DebugDrawEncoder() { DEBUG_DRAW_ENCODER(init(s_dde.m_defaultEncoder)); }

DebugDrawEncoder::~DebugDrawEncoder() { DEBUG_DRAW_ENCODER(shutdown()); }
...
void DebugDrawEncoder::end() { DEBUG_DRAW_ENCODER(end()); }

void DebugDrawEncoder::push() { DEBUG_DRAW_ENCODER(push()); }

void DebugDrawEncoder::pop() { DEBUG_DRAW_ENCODER(pop()); }

void DebugDrawEncoder::setDepthTestLess(bool _depthTestLess) { DEBUG_DRAW_ENCODER(setDepthTestLess(_depthTestLess)); }
...
void DebugDrawEncoder::setColor(uint32_t _abgr) { DEBUG_DRAW_ENCODER(setColor(_abgr)); }

void DebugDrawEncoder::setLod(uint8_t _lod) { DEBUG_DRAW_ENCODER(setLod(_lod)); }

void DebugDrawEncoder::setWireframe(bool _wireframe) { DEBUG_DRAW_ENCODER(setWireframe(_wireframe)); }

void DebugDrawEncoder::setStipple(bool _stipple, float _scale, float _offset) { DEBUG_DRAW_ENCODER(setStipple(_stipple, _scale, _offset)); }

void DebugDrawEncoder::setSpin(float _spin) { DEBUG_DRAW_ENCODER(setSpin(_spin)); }

void DebugDrawEncoder::setTransform(const void* _mtx) { DEBUG_DRAW_ENCODER(setTransform(_mtx)); }

void DebugDrawEncoder::setTranslate(float _x, float _y, float _z) { DEBUG_DRAW_ENCODER(setTranslate(_x, _y, _z)); }

void DebugDrawEncoder::pushTransform(const void* _mtx) { DEBUG_DRAW_ENCODER(pushTransform(_mtx, 1)); }

void DebugDrawEncoder::popTransform() { DEBUG_DRAW_ENCODER(popTransform()); }

void DebugDrawEncoder::moveTo(float _x, float _y, float _z) { DEBUG_DRAW_ENCODER(moveTo(_x, _y, _z)); }

void DebugDrawEncoder::moveTo(const bx::Vec3& _pos) { DEBUG_DRAW_ENCODER(moveTo(_pos)); }

void DebugDrawEncoder::lineTo(float _x, float _y, float _z) { DEBUG_DRAW_ENCODER(lineTo(_x, _y, _z)); }

void DebugDrawEncoder::lineTo(const bx::Vec3& _pos) { DEBUG_DRAW_ENCODER(lineTo(_pos)); }

void DebugDrawEncoder::close() { DEBUG_DRAW_ENCODER(close()); }

void DebugDrawEncoder::draw(const bx::Aabb& _aabb) { DEBUG_DRAW_ENCODER(draw(_aabb)); }

void DebugDrawEncoder::draw(const bx::Cylinder& _cylinder) { DEBUG_DRAW_ENCODER(draw(_cylinder, false)); }

void DebugDrawEncoder::draw(const bx::Capsule& _capsule) { DEBUG_DRAW_ENCODER(draw(*((const bx::Cylinder*)&_capsule), true)); }

void DebugDrawEncoder::draw(const bx::Disk& _disk) { DEBUG_DRAW_ENCODER(draw(_disk)); }

void DebugDrawEncoder::draw(const bx::Obb& _obb) { DEBUG_DRAW_ENCODER(draw(_obb)); }

void DebugDrawEncoder::draw(const bx::Sphere& _sphere) { DEBUG_DRAW_ENCODER(draw(_sphere)); }

void DebugDrawEncoder::draw(const bx::Triangle& _triangle) { DEBUG_DRAW_ENCODER(draw(_triangle)); }

void DebugDrawEncoder::draw(const bx::Cone& _cone) { DEBUG_DRAW_ENCODER(drawCone(_cone.pos, _cone.end, _cone.radius)); }

void DebugDrawEncoder::draw(GeometryHandle _handle) { DEBUG_DRAW_ENCODER(draw(_handle)); }
...
void DebugDrawEncoder::drawFrustum(const void* _viewProj) { DEBUG_DRAW_ENCODER(drawFrustum(_viewProj)); }
...
void DebugDrawEncoder::drawCone(const bx::Vec3& _from, const bx::Vec3& _to, float _radius) { DEBUG_DRAW_ENCODER(drawCone(_from, _to, _radius)); }
...
DebugDrawEncoderScopePush::DebugDrawEncoderScopePush(DebugDrawEncoder& _dde) : m_dde(_dde) { m_dde.push(); }

DebugDrawEncoderScopePush::~DebugDrawEncoderScopePush() { m_dde.pop(); }

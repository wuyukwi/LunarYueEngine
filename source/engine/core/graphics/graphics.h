...
namespace gfx
{
    ...
    /**/
    void vertex_pack(const float _input[4], bool _inputNormalized, attribute _attr, const vertex_layout& _decl, void* _data, uint32_t _index = 0);

    /**/
    void vertex_unpack(float _output[4], attribute _attr, const vertex_layout& _decl, const void* _data, uint32_t _index = 0);

    /**/
    void vertex_convert(const vertex_layout& _destDecl, void* _destData, const vertex_layout& _srcDecl, const void* _srcData, uint32_t _num = 1);

    /**/
    uint16_t weld_vertices(uint16_t* _output, const vertex_layout& _decl, const void* _data, uint16_t _num, float _epsilon = 0.001f);
    ...
    /**/
    const memory_view* make_ref(const void* _data, uint32_t _size, release_fn _releaseFn = nullptr, void* _userData = nullptr);
    ...
    /**/
    vertex_buffer_handle create_vertex_buffer(const memory_view* _mem, const vertex_layout& _decl, uint16_t _flags = BGFX_BUFFER_NONE);
    ...
    /**/
    dynamic_vertex_buffer_handle create_dynamic_vertex_buffer(uint32_t _num, const vertex_layout& _decl, uint16_t _flags = BGFX_BUFFER_NONE);

    /**/
    dynamic_vertex_buffer_handle
    create_dynamic_vertex_buffer(const memory_view* _mem, const vertex_layout& _decl, uint16_t _flags = BGFX_BUFFER_NONE);
    ...
    /**/
    frame_buffer_handle
    create_frame_buffer(backbuffer_ratio _ratio, texture_format _format, uint64_t _textureFlags = BGFX_SAMPLER_U_CLAMP | BGFX_SAMPLER_V_CLAMP);
    ...
    /**/
    void set_view_clear(view_id _id, uint16_t _flags, uint32_t _rgba = 0x000000ff, float _depth = 1.0f, uint8_t _stencil = 0);
    ...
    /**/
    void submit(view_id _id, program_handle _program, occlusion_query_handle _occlusionQuery, int32_t _depth = 0, bool _preserveState = false);
    ...
    /**/
    void set_image(uint8_t _stage, texture_handle _handle, uint8_t _mip, access _access, texture_format _format = texture_format::Count);

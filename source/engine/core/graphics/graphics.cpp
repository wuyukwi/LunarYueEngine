...
namespace gfx
{
    ...
    void set_trace_logger(const std::function<void(const std::string&)>& logger) { s_loggers["trace"] = logger; }

    void set_info_logger(const std::function<void(const std::string&)>& logger) { s_loggers["info"] = logger; }
    static std::function<void(const std::string&)> warning_logger;
    void set_warning_logger(const std::function<void(const std::string&)>& logger) { s_loggers["warning"] = logger; }
    static std::function<void(const std::string&)> error_logger;
    void set_error_logger(const std::function<void(const std::string&)>& logger) { s_loggers["error"] = logger; }
    ...
    struct gfx_callback : public bgfx::CallbackI
    {
        ...
        void profilerBeginLiteral(const char* /*_name*/, std::uint32_t /*_abgr*/, const char* /*_filePath*/, std::uint16_t /*_line*/) final {}
        ...
        bool cacheRead(std::uint64_t /*_id*/, void* /*_data*/, std::uint32_t /*_size*/) final { return false; }
        ...
        void
        captureBegin(std::uint32_t /*_width*/, std::uint32_t /*_height*/, std::uint32_t /*_pitch*/, texture_format /*_format*/, bool /*_yflip*/) final
        {}
    ...
    uint32_t
    topology_convert(topology_conversion _conversion, void* _dst, uint32_t _dstSize, const void* _indices, uint32_t _numIndices, bool _index32)
    {
    ...
    uint8_t get_supported_renderers(uint8_t _max, renderer_type* _enum) { return bgfx::getSupportedRenderers(_max, _enum); }

    const char* get_renderer_name(renderer_type _type) { return bgfx::getRendererName(_type); }

    void reset(uint32_t _width, uint32_t _height, uint32_t _flags) { bgfx::reset(_width, _height, _flags); }
    ...
    const memory_view* make_ref(const void* _data, uint32_t _size) { return bgfx::makeRef(_data, _size); }
    ...
    void dbg_text_printf(uint16_t _x, uint16_t _y, uint8_t _attr, const char* _format) { bgfx::dbgTextPrintf(_x, _y, _attr, _format); }
    ...
    index_buffer_handle create_index_buffer(const memory_view* _mem, uint16_t _flags) { return bgfx::createIndexBuffer(_mem, _flags); }
    ...
    dynamic_index_buffer_handle create_dynamic_index_buffer(uint32_t _num, uint16_t _flags) { return bgfx::createDynamicIndexBuffer(_num, _flags); }
    ...
    void update(dynamic_index_buffer_handle _handle, uint32_t _startIndex, const memory_view* _mem) { bgfx::update(_handle, _startIndex, _mem); }
    ...
    void update(dynamic_vertex_buffer_handle _handle, uint32_t _startVertex, const memory_view* _mem) { bgfx::update(_handle, _startVertex, _mem); }

    void destroy(dynamic_vertex_buffer_handle _handle) { bgfx::destroy(_handle); }

    uint32_t get_avail_transient_index_buffer(uint32_t _num) { return bgfx::getAvailTransientIndexBuffer(_num); }

    uint32_t get_avail_transient_vertex_buffer(uint32_t _num, const vertex_layout& _decl) { return bgfx::getAvailTransientVertexBuffer(_num, _decl); }

    uint32_t get_avail_instance_data_buffer(uint32_t _num, uint16_t _stride) { return bgfx::getAvailInstanceDataBuffer(_num, _stride); }

    void alloc_transient_index_buffer(transient_index_buffer* _tib, uint32_t _num) { bgfx::allocTransientIndexBuffer(_tib, _num); }
    ...
    indirect_buffer_handle create_indirect_buffer(uint32_t _num) { return bgfx::createIndirectBuffer(_num); }
    ...
    void get_uniform_info(uniform_handle _handle, uniform_info& _info) { bgfx::getUniformInfo(_handle, _info); }
    ...
    program_handle create_program(shader_handle _csh, bool _destroyShaders) { return bgfx::createProgram(_csh, _destroyShaders); }
    ...
    texture_handle
    create_texture_cube(uint16_t _size, bool _hasMips, uint16_t _numLayers, texture_format _format, uint64_t _flags, const memory_view* _mem)
    {
    ...
    uint32_t read_texture(texture_handle _handle, void* _data, uint8_t _mip) { return bgfx::readTexture(_handle, _data, _mip); }
    ...
    texture_handle get_texture(frame_buffer_handle _handle, uint8_t _attachment) { return bgfx::getTexture(_handle, _attachment); }

    void destroy(frame_buffer_handle _handle) { bgfx::destroy(_handle); }

    uniform_handle create_uniform(const char* _name, uniform_type _type, uint16_t _num) { return bgfx::createUniform(_name, _type, _num); }
    ...
    occlusion_query_result get_result(occlusion_query_handle _handle, int32_t* _result) { return bgfx::getResult(_handle, _result); }
    ...
    void set_palette_color(uint8_t _index, const float _rgba[]) { bgfx::setPaletteColor(_index, _rgba); }

    void set_palette_color(uint8_t _index, float _r, float _g, float _b, float _a) { bgfx::setPaletteColor(_index, _r, _g, _b, _a); }

    void set_view_name(view_id _id, const char* _name) { bgfx::setViewName(_id, _name); }

    void set_view_rect(view_id _id, uint16_t _x, uint16_t _y, uint16_t _width, uint16_t _height) { bgfx::setViewRect(_id, _x, _y, _width, _height); }

    void set_view_rect(view_id _id, uint16_t _x, uint16_t _y, backbuffer_ratio _ratio) { bgfx::setViewRect(_id, _x, _y, _ratio); }
    ...
    void set_view_frame_buffer(view_id _id, frame_buffer_handle _handle) { bgfx::setViewFrameBuffer(_id, _handle); }

    void set_view_transform(view_id _id, const void* _view, const void* _proj) { bgfx::setViewTransform(_id, _view, _proj); }

    void set_view_order(view_id _id, uint16_t _num, const view_id* _order) { bgfx::setViewOrder(_id, _num, _order); }
    ...
    void set_condition(occlusion_query_handle _handle, bool _visible) { bgfx::setCondition(_handle, _visible); }

    void set_stencil(uint32_t _fstencil, uint32_t _bstencil) { bgfx::setStencil(_fstencil, _bstencil); }

    uint16_t set_scissor(uint16_t _x, uint16_t _y, uint16_t _width, uint16_t _height) { return bgfx::setScissor(_x, _y, _width, _height); }
    ...
    uint32_t alloc_transform(transform* _transform, uint16_t _num) { return bgfx::allocTransform(_transform, _num); }

    void set_transform(uint32_t _cache, uint16_t _num) { bgfx::setTransform(_cache, _num); }

    void set_uniform(uniform_handle _handle, const void* _value, uint16_t _num) { bgfx::setUniform(_handle, _value, _num); }
    ...
    void set_buffer(uint8_t _stage, index_buffer_handle _handle, access _access) { bgfx::setBuffer(_stage, _handle, _access); }

    void set_buffer(uint8_t _stage, vertex_buffer_handle _handle, access _access) { bgfx::setBuffer(_stage, _handle, _access); }

    void set_buffer(uint8_t _stage, dynamic_index_buffer_handle _handle, access _access) { bgfx::setBuffer(_stage, _handle, _access); }

    void set_buffer(uint8_t _stage, dynamic_vertex_buffer_handle _handle, access _access) { bgfx::setBuffer(_stage, _handle, _access); }

    void set_buffer(uint8_t _stage, indirect_buffer_handle _handle, access _access) { bgfx::setBuffer(_stage, _handle, _access); }
    ...
    void request_screen_shot(frame_buffer_handle _handle, const char* _filePath) { bgfx::requestScreenShot(_handle, _filePath); }

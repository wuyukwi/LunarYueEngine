...
namespace gfx
{
    ...
    frame_buffer::frame_buffer(std::uint16_t _width, std::uint16_t _height, texture_format _format, std::uint32_t _textureFlags) :
    ...
    frame_buffer::frame_buffer(void* _nwh, uint16_t _width, uint16_t _height, texture_format _format, texture_format _depth_format)
    {
    ...
    const fbo_attachment& frame_buffer::get_attachment(std::uint32_t index) const { return textures_[index]; }

    const std::shared_ptr<texture>& frame_buffer::get_texture(uint32_t index) const { return get_attachment(index).texture; }

...
namespace gfx
{
    ...
    std::shared_ptr<frame_buffer> render_view::get_fbo(const std::string& id, const std::vector<std::shared_ptr<texture>>& bind_textures)
    {
        fbo_key key;
        key.id       = id;
    ...
    {
        static auto format =
            get_best_format(BGFX_CAPS_FORMAT_TEXTURE_FRAMEBUFFER, format_search_flags::requires_depth | format_search_flags::requires_stencil);
    ...
    {
        static auto format =
            get_best_format(BGFX_CAPS_FORMAT_TEXTURE_FRAMEBUFFER, format_search_flags::four_channels | format_search_flags::requires_alpha);
    ...
    std::shared_ptr<frame_buffer> render_view::get_g_buffer_fbo(const usize32_t& viewport_size)
    {
        ...
        static auto normal_format =
            get_best_format(BGFX_CAPS_FORMAT_TEXTURE_FRAMEBUFFER,
                            format_search_flags::four_channels | format_search_flags::requires_alpha | format_search_flags::half_precision_float);

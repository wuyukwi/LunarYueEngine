...
namespace gfx
{
    ...
    void
    render_pass::clear(std::uint16_t _flags, std::uint32_t _rgba /*= 0x000000ff */, float _depth /*= 1.0f */, std::uint8_t _stencil /*= 0*/) const
    {
        set_view_clear(id, _flags, _rgba, _depth, _stencil);
        touch();
    }

    void render_pass::clear() const { clear(BGFX_CLEAR_COLOR | BGFX_CLEAR_DEPTH | BGFX_CLEAR_STENCIL, 0x000000FF, 1.0f, 0); }

    ...
    private:
        ...
        public:
            explicit cmd_base(const std::string& name,
                              ...
                              bool               is_variadic) :
                name(name),
                command(!name.empty() ? "-" + name : ""), alternative(!alternative.empty() ? "--" + alternative : ""), description(description),
                required(required), handled(false), arguments({}), dominant(dominant), variadic(is_variadic)

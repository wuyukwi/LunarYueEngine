#pragma once
#include "../common_lib/hpp/type_traits.hpp"
#include "../common_lib/hpp/utility.hpp"
...
namespace core
{
    ...
    namespace async
    {
        namespace detail
        {
            ...
            template<typename T>
            using has_wait_until = decltype(std::declval<T>().wait_until(std::declval<std::chrono::system_clock::time_point>()));

            namespace v2
            {

                template<typename T>
                using is_future_impl = hpp::conjunction<hpp::is_detected<has_get, T>,
                                                        hpp::is_detected<has_wait, T>,
                                                        hpp::is_detected<has_valid, T>,
                                                        hpp::is_detected<has_wait_for, T>,

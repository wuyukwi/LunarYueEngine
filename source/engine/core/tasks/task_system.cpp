...
#include "../common_lib/platform/thread.hpp"
#include <limits>
...
namespace core
{
    ...
    task_system::task_queue::task_queue(task_system::task_queue&& other) noexcept : tasks_(std::move(other.tasks_)), done_(other.done_.load()) {}
    ...
    bool task_system::task_queue::cancel(uint64_t id)
    {
        ...
        {
            std::unique_lock<std::mutex> lock(mutex_);
            tasks_.erase(std::remove_if(std::begin(tasks_),
                                        std::end(tasks_),
    ...
    task_system::task_system(bool wait_on_destruct) : task_system(wait_on_destruct, std::thread::hardware_concurrency()) {}

    task_system::task_system(bool wait_on_destruct, std::size_t nthreads) : threads_count_ {nthreads}, wait_on_destruct_(wait_on_destruct)
    {

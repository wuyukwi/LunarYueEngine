...
namespace audio
{
    namespace priv
    {
        ...
        void source_impl::set_loop(bool on) { al_check(alSourcei(handle_, AL_LOOPING, on ? AL_TRUE : AL_FALSE)); }
        ...
        void source_impl::set_position(const float3& position) { al_check(alSourcefv(handle_, AL_POSITION, position.data())); }

        void source_impl::set_velocity(const float3& velocity) { al_check(alSourcefv(handle_, AL_VELOCITY, velocity.data())); }
        ...
        void source_impl::set_volume_rolloff(float rolloff) { al_check(alSourcef(handle_, AL_ROLLOFF_FACTOR, rolloff)); }
        ...
        void source_impl::enqueue_buffer(source_impl::native_handle_type h) { alSourceQueueBuffers(handle_, 1, &h); }

        source_impl::native_handle_type source_impl::native_handle() const { return handle_; }

        uintptr_t source_impl::get_bound_sound_uid() const { return reinterpret_cast<uintptr_t>(bound_sound_); }

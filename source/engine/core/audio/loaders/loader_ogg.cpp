...
namespace audio
{
    ...
    bool load_ogg_from_memory(const std::uint8_t* data, std::size_t data_size, sound_data& result, std::string& err)
    {
        if (!data)
        {
        ...
        std::size_t data_bytes = (stream_len_in_samples * std::size_t(info.channels)) * result.info.bytes_per_sample;
        ...
        stb_vorbis_get_samples_short_interleaved(oss, info.channels, reinterpret_cast<std::int16_t*>(result.data.data()), int(data_bytes));

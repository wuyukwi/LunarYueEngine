...
namespace audio
{
    ...
    struct wav_header
    {
        constexpr static const std::size_t spec_sz = riff_header::spec_sz + format_header::spec_sz + data_header::spec_sz;
        //------------------
        // RIFF Header
        //------------------
        riff_header riff;
    ...
    static bool read_header(wav_header& header, const std::uint8_t* data)
    {
        ...
        std::size_t offset_to_data = sizeof(header.format.header) + sizeof(header.format.fmt_chunk_size) + size_t(header.format.fmt_chunk_size);
    ...
    bool load_wav_from_memory(const std::uint8_t* data, std::size_t data_size, sound_data& result, std::string& err)
    {
        if (!data)
        {
        ...
        result.info.duration =
            sound_info::duration_t(sound_info::duration_t::rep(header.data.data_bytes) / sound_info::duration_t::rep(header.format.byte_rate));

...
namespace imguizmo
{
    ...
    static const char* translationInfoMask[] =
        {"X : %5.3f", "Y : %5.3f", "Z : %5.3f", "X : %5.3f Y : %5.3f", "Y : %5.3f Z : %5.3f", "X : %5.3f Z : %5.3f", "X : %5.3f Y : %5.3f Z : %5.3f"};
    ...
    void set_draw_list() { gContext.mDrawList = ImGui::GetWindowDrawList(); }
    ...
    bool is_using() { return gContext.mbUsing || gContext.mbUsingBounds; }

    bool is_over() { return (GetMoveType(NULL) != NONE) || GetRotateType() != NONE || GetScaleType() != NONE || is_using(); }

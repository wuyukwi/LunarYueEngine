...
namespace ImGui
{
    ...
    bool ImageButtonWithAspectAndTextDOWN(ImTextureID        texId,
    ...
    void RenderFrameEx(ImVec2 p_min, ImVec2 p_max, bool border, float rounding, float thickness)
    {
        ...
        if (border)
        {
            window->DrawList->AddRect(p_min + ImVec2(1, 1), p_max + ImVec2(1, 1), GetColorU32(ImGuiCol_BorderShadow), rounding, 0, thickness);
    ...
    static void PushMultiItemsWidthsAndLabels(const char* labels[], int components, float w_full)
    {
        ...
        const float w_item_one =
            ImMax(1.0f, (w_full - (style.ItemInnerSpacing.x * 2.0f) * (components - 1)) / (float)components) - style.ItemInnerSpacing.x;
        for (int i = 0; i < components; i++)
            window->DC.ItemWidthStack.push_back(w_item_one - CalcTextSize(labels[i]).x);
        window->DC.ItemWidth = window->DC.ItemWidthStack.back();
    }

    bool DragFloatNEx(const char*      labels[],
                      float*           v,
                      int              components,
                      float            v_speed,
                      float            v_min,
    ...
    bool DragUIntNEx(const char*      labels[],
                     unsigned int*    v,
                     int              components,
                     float            v_speed,
                     unsigned int     v_min,
                     ...
                     ImGuiSliderFlags flags)
    {
        ...
        for (int i = 0; i < components; i++)
        {
            ...
            value_changed |= DragInt("", &val, v_speed, static_cast<int>(v_min), static_cast<int>(v_max), display_format, flags);
            v[i] = val;
            SameLine(0, g.Style.ItemInnerSpacing.x);
            PopID();
            PopID();
    ...
    bool
    DragIntNEx(const char* labels[], int* v, int components, float v_speed, int v_min, int v_max, const char* display_format, ImGuiSliderFlags flags)
    {
        ImGuiWindow* window = GetCurrentWindow();
        if (window->SkipItems)
            return false;
    ...
    bool ToolbarButton(const char* id, ImTextureID texture, const char* tooltip, bool selected, bool enabled)
    {
        return ImageButtonEx(id, texture, ImVec2(24, 24), tooltip, selected, enabled);
    }

    bool ImageButtonEx(const char* id, ImTextureID texture, ImVec2 size, const char* tooltip, bool selected, bool enabled)
    {
        ...
        {
            if (ImageButton(id, texture, size, ImVec2(0, 0), ImVec2(1, 1), bg_color))
            {
                ret = true;
            }
        }
    ...
    int ImageButtonWithAspectAndLabel(ImTextureID         texture,
                                      ImVec2              texture_size,
                                      ImVec2              size,
                                      ImVec2              uv0,
                                      ImVec2              uv1,
    ...
    bool BeginToolbar(const char* str_id, ImVec2 screen_pos, ImVec2 size)
    {
        ...
        ImGuiWindowFlags flags = ImGuiWindowFlags_NoTitleBar | ImGuiWindowFlags_NoMove | ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoScrollbar |

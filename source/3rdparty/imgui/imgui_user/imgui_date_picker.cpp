...
namespace ImGui
{
                     ...
                     const char* downArrowString)
    {
        ...
        static char*     pMonthNames[12]                = {&monthNames[0][0],
                                                           &monthNames[1][0],
                                                           &monthNames[2][0],
                                                           &monthNames[3][0],
                                                           &monthNames[4][0],
        ...
        if (strlen(dayNames[0]) == 0)
        {
            ...
            for (int i = 0; i < 12; i++)
            {
                ...
                {
                    static const char* fallbacks[12] = {
        ...
        const float widthRequiredByCalendar = 2.f * arrowLeftWidth + 2.f * arrowRightWidth + ImGui::CalcTextSize(monthNames[maxMonthWidthIndex]).x +
        ...
        const float w =
            widthRequiredByCalendar > widthRequiredByTextBox ? widthRequiredByCalendar : widthRequiredByTextBox; // ImGui::CalcItemWidth();
        ...
        ImGui::SameLine(ImGui::GetContentRegionAvail().x - yearPartWidth - ImGui::GetStyle().WindowPadding.x - ImGui::GetStyle().ItemSpacing.x * 4.f);
        ImGui::PushID(1235);
        if (ImGui::Button(arrowLeft))
        {
            d.tm_year -= 1;
        ...
        for (int dw = 0; dw < 7; dw++)
        {
            ...
            ImGui::EndGroup();
            if (dw != 6)
                ImGui::SameLine(ImGui::GetContentRegionAvail().x - (6 - dw) * (ImGui::GetContentRegionAvail().x / 7.f));
        }
    ...
    void
    TestDateChooser(const char* dateFormat, const char* leftArrow, const char* rightArrow, const char* upArrowString, const char* downArrowString)
    {
        static tm date = GetDateZero();
        if (DateChooser("DateChooser##id", date, dateFormat, NULL, leftArrow, rightArrow, upArrowString, downArrowString))
        {

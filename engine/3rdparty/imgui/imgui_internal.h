...
struct ImGuiNextItemData
{
    ...
    ImGuiID                     FocusScopeId;   // Set by SetNextItemMultiSelectData() (!= 0 signify value has been set, so it's an LunarYue version of HasSelectionData, we don't use Flags for this because they are cleared too early. This is mostly used for debugging)
    ImGuiCond                   OpenCond;
...
enum ImGuiNavMoveFlags_
{
    ...
    ImGuiNavMoveFlags_AlsoScoreVisibleSet   = 1 << 5,   // Store LunarYue result in NavMoveResultLocalVisible that only comprise elements that are already fully visible (used by PageUp/PageDown)

...
#define TRY_GATHER(type, output) \
    if (auto plugin = GetPlugin<LunarYue::UI::Plugins::DataDispatcher<type>>(); plugin) \
...
namespace LunarYue::UI::Plugins
{
    ...
    public:
         ...
         * @param p_provider
         */
        void RegisterProvider(std::function<void(T)> p_provider) { m_provider = p_provider; }

        /**
         * Register a gather (Which function should be called when the widget data needs to be updated)
         * @param p_provider
         */
        void RegisterGatherer(std::function<T(void)> p_gatherer) { m_gatherer = p_gatherer; }
         ...
         */
        void NotifyChange() { m_valueChanged = true; }

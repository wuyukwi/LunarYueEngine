...
#define TRY_GATHER(type, output)	if (auto plugin = GetPlugin<LunarYue::UI::Plugins::DataDispatcher<type>>(); plugin) output = plugin->Gather();
#define TRY_PROVIDE(type, output)	if (auto plugin = GetPlugin<LunarYue::UI::Plugins::DataDispatcher<type>>(); plugin) plugin->Provide(output);
#define TRY_NOTIFY_CHANGE(type)		if (auto plugin = GetPlugin<LunarYue::UI::Plugins::DataDispatcher<type>>(); plugin) plugin->NotifyChange();

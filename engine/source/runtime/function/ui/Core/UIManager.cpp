...
LunarYue::UI::Core::UIManager::UIManager() {}

LunarYue::UI::Core::UIManager::~UIManager() {}

void LunarYue::UI::Core::UIManager::initialize(WindowUIInitInfo init_info, EditorStyle p_style)
{
    ...
    // デフォルトフォントをロード
    m_fonts["default"] =
        io.Fonts->AddFontFromFileTTF(init_info.config_manager->getEditorFontPath().generic_string().data(), font_size, nullptr, icon_ranges.Data);

    // Font Awesome フォントをロード
    std::filesystem::path fa_font_path = init_info.config_manager->getEditorFontPath().parent_path() / "fa-solid-900.ttf";
    ...
    std::string big_icon_path_string   = init_info.config_manager->getEditorBigIconPath().generic_string();
    std::string small_icon_path_string = init_info.config_manager->getEditorSmallIconPath().generic_string();
...
void LunarYue::UI::Core::UIManager::ApplyStyle(EditorStyle p_style)
{
...
bool LunarYue::UI::Core::UIManager::LoadFont(const std::string& p_id, const std::string& p_path, float p_fontSize)
{
...
bool LunarYue::UI::Core::UIManager::UnloadFont(const std::string& p_id)
{
    if (m_fonts.find(p_id) != m_fonts.end())
    {
        m_fonts.erase(p_id);
...
bool LunarYue::UI::Core::UIManager::UseFont(const std::string& p_id)
{
...
void LunarYue::UI::Core::UIManager::UseDefaultFont() { ImGui::GetIO().FontDefault = nullptr; }

void LunarYue::UI::Core::UIManager::EnableEditorLayoutSave(bool p_value)
{
    if (p_value)
...
bool LunarYue::UI::Core::UIManager::IsEditorLayoutSaveEnabled() const { return ImGui::GetIO().IniFilename != nullptr; }

void LunarYue::UI::Core::UIManager::SetEditorLayoutSaveFilename(const std::string& p_filename)
{
...
void LunarYue::UI::Core::UIManager::SetEditorLayoutAutosaveFrequency(float p_frequency) { ImGui::GetIO().IniSavingRate = p_frequency; }

float LunarYue::UI::Core::UIManager::GetEditorLayoutAutosaveFrequency(float p_frequeny) { return ImGui::GetIO().IniSavingRate; }

void LunarYue::UI::Core::UIManager::EnableDocking(bool p_value)
{
...
void LunarYue::UI::Core::UIManager::ResetLayout(const std::string& p_config) const { ImGui::LoadIniSettingsFromDisk(p_config.c_str()); }

bool LunarYue::UI::Core::UIManager::IsDockingEnabled() const { return m_dockingState; }

void LunarYue::UI::Core::UIManager::SetCanvas(Modules::Canvas& p_canvas)
{
    RemoveCanvas();
...
void LunarYue::UI::Core::UIManager::RemoveCanvas() { m_currentCanvas = nullptr; }

void LunarYue::UI::Core::UIManager::Render()
{
    if (m_currentCanvas)
    {
...
void LunarYue::UI::Core::UIManager::PushCurrentFont() {}

void LunarYue::UI::Core::UIManager::PopCurrentFont() {}
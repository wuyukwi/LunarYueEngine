...
namespace LunarYue
{
    namespace
    {
        ...
        std::shared_ptr<RawBone> createBone(SkeletonData* skeleton_data, int parent_index)
        {
            ...
            std::shared_ptr<RawBone> bone_ptr = std::make_shared<LunarYue::RawBone>();
            *bone_ptr                         = raw_bone;
    ...
    std::shared_ptr<LunarYue::AnimationClip> AnimationLoader::loadAnimationClipData(std::string animation_clip_url)
    {
        AnimationAsset animation_clip;
        g_runtime_global_context.m_asset_manager->loadAsset(animation_clip_url, animation_clip);
        return std::make_shared<LunarYue::AnimationClip>(animation_clip.clip_data);
    }

    std::shared_ptr<LunarYue::SkeletonData> AnimationLoader::loadSkeletonData(std::string skeleton_data_url)
    {
        SkeletonData data;
        g_runtime_global_context.m_asset_manager->loadAsset(skeleton_data_url, data);
        return std::make_shared<LunarYue::SkeletonData>(data);
    }

    std::shared_ptr<LunarYue::AnimSkelMap> AnimationLoader::loadAnimSkelMap(std::string anim_skel_map_url)
    {
        AnimSkelMap data;
        g_runtime_global_context.m_asset_manager->loadAsset(anim_skel_map_url, data);
        return std::make_shared<LunarYue::AnimSkelMap>(data);
    }

    std::shared_ptr<LunarYue::BoneBlendMask> AnimationLoader::loadSkeletonMask(std::string skeleton_mask_file_url)
    {
        BoneBlendMask data;
        g_runtime_global_context.m_asset_manager->loadAsset(skeleton_mask_file_url, data);
        return std::make_shared<LunarYue::BoneBlendMask>(data);
    }

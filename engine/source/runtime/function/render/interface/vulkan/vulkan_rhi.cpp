...
namespace LunarYue
{
    ...
    void VulkanRHI::initialize(RHIInitInfo init_info)
    {
        ...
        // https://developer.apple.com/library/archive/documentation/Porting/Conceptual/PortingUnix/compiling/compiling.html
        char const* vk_layer_path    = LunarYue_XSTR(LunarYue_VK_LAYER_PATH);
        char const* vk_icd_filenames = LunarYue_XSTR(LunarYue_VK_ICD_FILENAMES);
        setenv("VK_LAYER_PATH", vk_layer_path, 1);
...
#error Unknown Compiler
    ...
    void VulkanRHI::createInstance()
    {
        ...
        appInfo.pApplicationName   = "LunarYue_renderer";
        appInfo.applicationVersion = VK_API_VERSION_1_1;
        appInfo.pEngineName        = "LunarYue";
                                            ...
                                            RHIPipeline*&                        pPipelines)
    {
        ...
        {
            LOG_ERROR("(specialization_map_entry_size_total == specialization_map_entry_current)&& (specialization_info_total == "
                      "specialization_info_current)");
            return false;
        }
    ...
    void
    VulkanRHI::cmdBeginRenderPassPFN(RHICommandBuffer* commandBuffer, const RHIRenderPassBeginInfo* pRenderPassBegin, RHISubpassContents contents)
    {
    ...
    {
        return _vkCmdDrawIndexed(
            ((VulkanCommandBuffer*)commandBuffer)->getResource(), indexCount, instanceCount, firstIndex, vertexOffset, firstInstance);
    }
                                         ...
                                         const RHICopyDescriptorSet*  pDescriptorCopies)
    {
        ...
        vkUpdateDescriptorSets(
            m_device, descriptorWriteCount, vk_write_descriptor_set_list.data(), descriptorCopyCount, vk_copy_descriptor_set_list.data());
    }
    ...
    void
    VulkanRHI::cmdDraw(RHICommandBuffer* commandBuffer, uint32_t vertexCount, uint32_t instanceCount, uint32_t firstVertex, uint32_t firstInstance)
    {
        ...
        for (uint32_t i = 0; i < k_max_frames_in_flight; i++)
        {
            ...
            if (vkCreateSemaphore(m_device, &semaphore_create_info, nullptr, &m_image_available_for_render_semaphores[i]) != VK_SUCCESS ||
                ...
                vkCreateSemaphore(m_device,
                                  ...
                                  nullptr,
                                  &(((VulkanSemaphore*)m_image_available_for_texturescopy_semaphores[i])->getResource())) != VK_SUCCESS ||
    ...
    RHISampler* VulkanRHI::getOrCreateDefaultSampler(RHIDefaultSamplerType type)
    {
        switch (type)
        {
            case LunarYue::Default_Sampler_Linear:
            ...
            case LunarYue::Default_Sampler_Nearest:
                                                 ...
                                                 VmaAllocationInfo*             pAllocationInfo)
    {
        ...
        VkResult result = vmaCreateBufferWithAlignment(
            allocator, &buffer_create_info, pAllocationCreateInfo, minAlignment, &vk_buffer, pAllocation, pAllocationInfo);
    ...
    void
    VulkanRHI::destroyDebugUtilsMessengerEXT(VkInstance instance, VkDebugUtilsMessengerEXT debugMessenger, const VkAllocationCallbacks* pAllocator)
    {
    ...
    LunarYue::QueueFamilyIndices VulkanRHI::findQueueFamilies(VkPhysicalDevice physicalm_device) // for device and surface
    {
    ...
    LunarYue::SwapChainSupportDetails VulkanRHI::querySwapChainSupport(VkPhysicalDevice physicalm_device)
    {

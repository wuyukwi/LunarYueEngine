...
namespace LunarYue
{
    ...
    public:
        ...
        GameObjectDesc(size_t go_id, std::vector<GameObjectPartDesc> parts) : m_go_id(go_id), m_object_parts(std::move(parts)) {}
...
template<>
struct std::hash<LunarYue::GameObjectPartId>
{
    size_t operator()(const LunarYue::GameObjectPartId& rhs) const noexcept { return rhs.getHashValue(); }
};

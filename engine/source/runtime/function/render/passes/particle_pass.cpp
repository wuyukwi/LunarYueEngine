...
namespace LunarYue
{
    ...
    void ParticlePass::setupDescriptorSetLayout()
    {
        ...
        // compute descriptor sets
        {
            ...
            {
                RHIDescriptorSetLayoutBinding& LunarYue_texture_layout_binding = particle_layout_bindings[10];
                LunarYue_texture_layout_binding.binding                        = 10;
                LunarYue_texture_layout_binding.descriptorType                 = RHI_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER;
                LunarYue_texture_layout_binding.descriptorCount                = 1;
                LunarYue_texture_layout_binding.stageFlags                     = RHI_SHADER_STAGE_COMPUTE_BIT;
            }
    ...
    void ParticlePass::updateDescriptorSet()
    {
        for (int eid = 0; eid < m_emitter_count; ++eid)
        {
            // compute part
            {
                ...
                RHIDescriptorImageInfo LunarYue_texture_image_info = {};
                LunarYue_texture_image_info.sampler                = sampler;
                LunarYue_texture_image_info.imageView              = m_LunarYue_logo_texture_image_view;
                LunarYue_texture_image_info.imageLayout            = RHI_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL;

                {
                    ...
                    descriptorset.pImageInfo             = &LunarYue_texture_image_info;
                    descriptorset.descriptorCount        = 1;
                }
    ...
    void ParticlePass::simulate()
    {
        for (auto i : m_emitter_tick_indices)
        {
            ...
            m_rhi->cmdDispatchIndirect(
                m_compute_command_buffer, m_emitter_buffer_batches[i].m_indirect_dispatch_argument_buffer, s_argument_offset_emit);
            ...
            m_rhi->cmdDispatchIndirect(
                m_compute_command_buffer, m_emitter_buffer_batches[i].m_indirect_dispatch_argument_buffer, s_argument_offset_simulate);
    ...
    void ParticlePass::updateEmitterTransform()
    {
        for (ParticleEmitterTransformDesc& transform_desc : m_emitter_transform_indices)
        {
            ...
            memcpy(
                m_emitter_buffer_batches[index].m_emitter_desc_mapped, &m_emitter_buffer_batches[index].m_emitter_desc, sizeof(ParticleEmitterDesc));
        }
    }

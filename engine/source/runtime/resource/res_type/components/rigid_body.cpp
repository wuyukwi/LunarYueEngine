...
namespace LunarYue
{
    RigidBodyShape::RigidBodyShape(const RigidBodyShape& res) : m_local_transform(res.m_local_transform)
    {
        if (res.m_geometry.getTypeName() == "Box")
        {
            ...
            m_geometry = LunarYue_REFLECTION_NEW(Box);
            LunarYue_REFLECTION_DEEP_COPY(Box, m_geometry, res.m_geometry);
        }
        else
        {
    ...
    RigidBodyShape::~RigidBodyShape() { LunarYue_REFLECTION_DELETE(m_geometry); }
} // namespace LunarYue